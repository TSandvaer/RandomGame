...
- **Standing on a circular stone base** integrated into the arena floor — the construct **repositions ONLY via phase-blink between cast volleys; it never walks, chases, or pursues** (boss design lock, AMENDED 2026-05-30 — see §5.5a below). The original lock read "never moves from its plinth"; Sponsor's 2026-05-30 soak found the fully-static read too kitable, so the construct now phase-shifts between a small set of fixed plinth-points. This is still a *phase-shift sentinel, NOT a mobile melee boss* — it teleports, it does not travel. See §5.5a Phase-Blink amendment + boss-arena note below.
...
**Boss-arena note (AMENDED 2026-05-30):** the Sentinel does not WALK — combat happens around a construct that **blinks between fixed plinth-points**, never travels across the floor. Drew's boss-room scene authoring should reflect this (arena ~32×24 tiles, room ports at four cardinal directions for player navigation but ports stay LOCKED during fight per `room-gate` convention). Instead of ONE central plinth, the arena carries **4 candidate plinth-points** (one central + three at roughly the 10-, 2-, and 6-o'clock positions around the room interior, each well inside the locked ports so the player always has dodge-room). The Sentinel occupies one at a time and phase-blinks to another between cast volleys. This is still a deliberate departure from Stratum1Boss's mobile-melee design — the construct's stillness-between-blinks IS its tonal beat (it has waited in one pose for centuries; the blink is a rare, jarring cue that it has noticed you and is re-fixing its gaze). See §5.5a for the full phase-blink visual + cadence spec.
...
### §5.5a — Archive Sentinel phase-blink reposition (AMENDED 2026-05-30, Sponsor soak revision)

**Why this amendment exists.** Sponsor soaked build `38e0ecb` (2026-05-30) and found the fully-stationary Sentinel too passive — it casts but never moves, so a player simply walks to a safe angle and out-paces every volley. The original design lock ("never moves from its plinth," §5.5 silhouette + boss-arena note) was tonally right but kitable. **Sponsor's chosen revision:** the Sentinel **repositions between cast volleys** — NOT by walking/chasing, but by **phase-shifting (blinking/teleporting) to a different plinth-point.** The "phase-shift sentinel" name in the original lock is the hook; this amendment leans into it. The construct still never travels across the floor under its own feet — it ceases-to-be-here and re-coheres-there.

**1. Framing confirm.** Blink-reposition is the *correct* expression of the phase-shift identity, not a compromise of it. A construct the scholars built FROM the archive, animated by embergrave, would not lumber — it would un-fix and re-fix its position the way a thought jumps a page. The blink reinforces "this is a made thing reading you, not a beast hunting you": it relocates to keep its open-book gaze on the player, the same way a reader turns to follow movement across a room. Walking would have made it a worse S1 Boss; blinking makes it a *more* legible Sentinel. Lock holds, now reads "repositions ONLY via phase-blink, never walks/chases."

**2. Visual treatment of the blink — must read as "phase-shift," not generic teleport.** Three-beat effect, total ~520 ms, both endpoints telegraph:

- **Departure (≈140 ms) — ember-dissolve + archive-glyph scatter.** The construct's stone-bone body dissolves UP into rising ember motes (`#FF6A2A`, the player-flame / vein-core hex — diegetic: it returns to the substance it's made of), WHILE 4-6 small parchment-tan `#A89270` "archive-glyph" flecks scatter outward from the book-eye (reads as "pages losing their place"). The book-eye is the LAST thing to dissolve — pages flare to the brightest `#FF6A2A` for ~50 ms (impact-frame contrast per `.claude/docs/html5-export.md` § "Burst contrast against high-hue-saturation same-z sprites") then wink out. Sprite goes to `modulate.a = 0` over the dissolve; do NOT just hide() — the alpha-fade is the read.
- **Travel gap (≈80 ms) — empty.** Neither plinth shows the body. A faint `#C25A1F` ash-glow vein-mid shimmer runs along the floor seam BETWEEN departure and arrival points (a 2-px `_draw()` polyline, NOT Polygon2D — per `.claude/docs/html5-export.md` § "Shape OUTLINES" + uma hard rule) so the eye is led to the arrival point. This is the only cue that "it went *that* way."
- **Arrival (≈140 ms + ≈160 ms arrival-telegraph pre-roll, see item 3) — reform-in.** Ember motes converge DOWN onto the destination plinth and re-cohere into the body (reverse of departure); book-eye re-ignites LAST, settling from bright `#FF6A2A` back to rest `#D24A3C` aggro-tone. Brass clamps catch a 1-frame `#FF6A2A` glint as they re-seat.

  **Color channels (all sub-1.0, HTML5 HDR-clamp safe per `.claude/docs/html5-export.md`):** ember motes `#FF6A2A` = `Color(1.0, 0.416, 0.165)`; archive-glyph flecks `#A89270` = `Color(0.659, 0.573, 0.439)`; floor-seam shimmer `#C25A1F` = `Color(0.761, 0.353, 0.122)`; impact-flare peak uses the EXISTING `AFTERSHOCK_FLASH_WHITE`-class warm-white `#FFF2BF` = `Color(1.0, 0.949, 0.749)` for the ≤50 ms book-eye peak only (every channel < 1.0 — verified). No new constant needed; reuse the PR #291 v7 warm-white. Particles via room-parented `call_deferred("add_child", burst)` per `.claude/docs/combat-architecture.md` § "Room-parented CPUParticles2D burst."

  **Both endpoints telegraph** (departure dissolve + arrival reform are both visible, with the floor-seam shimmer bridging) — this is deliberate so the blink reads as a *traversal*, not an instant cut. A generic teleport (instant pop) would read as a bug; the dissolve→shimmer→reform sells "phase-shift."

**3. Destination constraints — bounded, telegraphed-before-commit.** Blink is **arena-bounded, NOT arena-wide free-warp**: destination is always one of the **4 fixed candidate plinth-points** (§5.5 boss-arena note — central + 10/2/6-o'clock interior positions, all well inside the locked ports). Never blinks adjacent to the player (enforce a min-distance: pick the candidate FARTHEST from the player's current position, or random-among-the-two-farthest to avoid predictability). **Arrival telegraphs ~160 ms BEFORE the body reforms:** at the chosen destination, the reform-in ember motes begin converging + a faint book-eye pre-glow appears ON the destination plinth during the travel-gap's tail. A skilled player reads the converging motes and can pre-position to punish the arrival (the Sentinel is briefly mid-reform = a read/punish window, mirroring the cast wind-up's fairness contract). NOT instant — the pre-roll is the skill-expression seam. The departure point's dissolve and the arrival point's pre-glow are BOTH on-screen for ~80 ms (the travel gap), so an attentive player sees where it's going before it's fully there.

**4. Cadence feel — blink on cast-recovery, gated, escalates at phase-transition.** Default: **blink once per completed cast volley**, fired at the tail of `post_fire_recovery` (NOT every individual bolt — a volley = the full cast→recovery cycle). To keep it threatening-not-spammy: (a) a **hard floor of ~2.5 s real-time between blinks** so back-to-back fast casts can't strobe the construct around the room; (b) the Sentinel does NOT blink if the player is already at max range (no point repositioning away from someone who's already far — it only blinks to *re-close gaze angle* or *deny a kited safe-spot*, so blink-suppress when player is beyond cast range and not moving). At **phase-transition (50% HP, Drew's threshold per §5.5)** the cadence tightens: blink-floor drops to ~1.5 s and the construct may blink mid-volley once (the phase-flip "the book has lost patience" beat). Pre-50% it's a slow, deliberate, once-per-volley relocation; post-50% it's a faster, more aggressive re-fix. This is the threat curve: early it's a positioning puzzle, late it's a pressure tool. Numeric tuning (exact floor seconds, mid-volley-blink count) is Drew's balance lever per the Appendix "S2 mob state-machine balance" exclusion — these are the *feel targets*, not locked constants.

**Cross-references for Drew:** `.claude/docs/html5-export.md` § "Shape OUTLINES" (floor-seam polyline via `_draw()`, NOT Polygon2D) + § "Burst contrast" (impact-flare warm-white frame) + § "HTML5 HDR clamp" (sub-1.0 channels verified above); `.claude/docs/combat-architecture.md` § "Room-parented CPUParticles2D burst" (ember-mote dissolve/reform bursts deferred-add to room) + § "physics-flush rule" (any Area2D/monitoring touch on blink must `call_deferred`); §5.5 boss-arena note (4 plinth-points layout). **OOS for this brief:** the GDScript blink state-machine wiring is Drew's; this spec is the visual + cadence contract he implements against.
...
## Coordination note — Archive Sentinel phase-blink amendment (2026-05-30)

§5.5a (Archive Sentinel phase-blink reposition) landed via a single amendment commit on 2026-05-30, reconciling:

- **Sponsor soak of build `38e0ecb` (2026-05-30)** — the Stratum2BossRoom + ArchiveSentinel boss (shipped W3-T7 Stage 5, PR #374, commit `30f5779`) read too passive: the boss casts but never moves, so it is trivially kited.
- **Sponsor decision 2026-05-30** — add "reposition between casts," expressed as a phase-blink (teleport between fixed plinth-points), NOT a walk/chase. Preserves the rooted-construct "never walks" identity; adds a movement beat via the phase-shift name already in the original lock.

The amendment is **additive and narrow**: it amends the two original "never moves from its plinth" claims (the §5.5 silhouette bullet + boss-arena note) to "repositions ONLY via phase-blink, never walks/chases," and adds §5.5a as the full visual + cadence spec Drew implements against. The §2 Cinder-Rust ramp, §6 daltonization, §7 tester checklist, and all §1-§1.6 hybrid framing are unchanged. GDScript blink implementation is OOS (Drew's next ticket); the cast-bolt visual (PR #380) and HP/zoom (QA'd) are unchanged.
