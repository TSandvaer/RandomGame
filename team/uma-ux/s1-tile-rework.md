...
Default: **upgrade existing where a regen suffices** (same file path, higher craft); call out genuinely-new needs. Tools per `pixellab-pipeline.md` (orch-session only — sub-agents lack PixelLab). Doctrine-lock Strategy 3.

### 5.1 TILES — the headline (UPGRADE, regen at higher craft)

- **Floor flagstone SET — UPGRADE `floor_sandstone.png` → 5 variants.** `create_topdown_tileset`. Was 1 flat tile; now a 5-variant crafted set (§2.2A). **The single most important gen.** ~1–2 gens + iteration.
- **Wall ashlar SET — UPGRADE `wall_cloister.png` → running-bond autotile + 2–3 block variants.** `create_topdown_tileset` (Wang/blob). Kills the vertical-joint wallpaper repeat. ~1–2 gens + iteration.
- **Floor accents — NEW: cracked-sunk flagstone (×1–2) + moss-creep flagstone (×1).** `create_topdown_tileset`/`create_map_object`. Repeat-breakers + floor material story. ~1 gen.
- **Wall niche accent — NEW: ×1 arched-niche block.** Demotes the old alcove motif to a rare accent. `create_map_object` or fold into wall set. ~1 gen.

### 5.2 PROPS — mostly REUSE; one possible upgrade

- **Pillar — UPGRADE `pillar_arch.png` (optional).** The colonnade puts 8 pillars on screen — if the existing 48×64 pillar reads thin at game zoom, regen at higher craft (more carved detail, clearer lit/shadow sides) so a row of them reads as a real colonnade. **Recommend a craft-regen** since pillars are now the hero prop. ~1 gen.
- **Braziers (lit + cold), rubble, parchment, banner, moss — REUSE as-is.** The 7 approved props already doctrine-lock cleanly; the #407 problem was *composition + tiles*, not these sprites. No regen unless a specific one reads poorly in the new composition at soak. **0 gens by default.**
- **2nd rubble silhouette — NICE-TO-HAVE (skip unless needed).** A long fallen-beam shape vs the round pile kills copy-paste read along the south band. ~1 gen IF the single rubble silhouette reads repetitive at soak.

### 5.3 Gen-count summary

- **MUST: ~3–5 tile gens** (floor set + wall set + floor accents + niche), iterated for premium craft. This is the headline; budget iteration here.
- **RECOMMEND: 1 pillar craft-regen** (hero prop in the colonnade).
- **OPTIONAL: 1 2nd-rubble-shape** only if soak shows copy-paste.
- **Total: ~4–7 gens.** Well within budget (`pixellab-pipeline.md` cost model — `create_topdown_tileset`/`create_map_object` are ~1 gen each; Tier 2 has ample headroom). Iterate tiles until they read crafted at game zoom; that's where the credits should go.
...
4. **Gen scope** (§5): ~4–7 PixelLab gens (tiles are the headline + iterate; pillar regen; props mostly reuse). Approve, or hero-mode the floor/wall for max craft?
