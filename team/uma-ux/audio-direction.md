...
### M3-T2-W3-T16b — `sfx-boss-kill-horn.ogg` (Beat F2 boss-defeated cinematic)

The Beat F2 horn cue (`audio/sfx/mobs/sfx-boss-kill-horn.ogg`, AD-23 tester check, ClickUp `86c9wjzgh`) ships as an **algorithmic placeholder** under the same disclosure pattern as the Cinder Vaults cues above. Composer script: `audio/_src/composer/compose_boss_kill_horn.py`. Reproducible: `python audio/_src/composer/compose_boss_kill_horn.py` from repo root regenerates the OGG deterministically (numpy RNG seed = 41).

- **Spec match (structural):** 0.900 s mono OGG Vorbis at 44.1 kHz, peak normalized to -3 dBFS — per `audio-direction.md §2` row sfx-boss-kill-horn (0.90 s, hand-composed) and `§4` SFX format rule (mono / q5-ish).
- **Tonal direction (Uma F2):** sustained warm horn note rising D3 (146.83 Hz) → E3 (164.81 Hz) across the 0.9 s — gentle 40 ms attack (no sting transient), envelope-peak lands at ~T+0.8 s as embers exit screen, 250 ms release tails into the title-card fade-in at T+1.2 s (silence-as-punctuation per `audio-architecture.md §"Tonal pattern — silence as punctuation"`). Brass harmonic stack matches the M3-T2-W1-T7 `sfx-boss-wake` stinger (low-brass at A2 fundamental for entry); the kill-horn at D3 lands a minor-third brighter so the boss's entry/exit harmonic palette reads as "darker → brighter lift" — the entrance and the exit rhyme but the climax pays off.
- **Tonally limited:** the synthesis emulates additive low-brass via 6-harmonic stack + brass tilt + 1100 Hz body filter + breath-noise sheen, but lacks the bow-pressure-modulation and chapel-tail reverb that a Spitfire LABS low-brass + real-room IR would carry.

**M4 promotion plan** (file under M4 audio polish backlog alongside the Cinder Vaults promotions): replace with either (a) Spitfire LABS low-brass sample at D3 with a tasteful 2-semitone pitch-glide to E3, ~1-2 h authoring + mix; OR (b) freesound CC0 source (alphorn / low-trombone sustained note at the right pitch register) + DAW mix to the same 0.9 s duration and -3 dBFS peak. Single-file replace at `audio/sfx/mobs/sfx-boss-kill-horn.ogg` per the §4 versioning rule (cue ID == filename; the file underneath can change without churning AD-23 or any code site).
