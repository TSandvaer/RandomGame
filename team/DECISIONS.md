...
- Affects: Devon (level-up math N1 must consume the boss HP target — first kill at character level 4 should land in ~6–8 minutes per AC4); Tess (acceptance test: AC #6 boss-defeatable validated by `tests/test_stratum1_boss.gd` + `tests/test_stratum1_boss_room.gd` — 30+ paired tests covering all 12 task-spec coverage points; soak: kill the boss after the entry sequence, verify loot drops + door unlocks); Uma (boss display name `WARDEN OF THE OUTER CLOISTER` matches `MobDef.display_name`; boss-defeated cinematic layer subscribes to `Stratum1BossRoom.boss_defeated` signal).
