...
## 2026-05-03 — Hitboxes always check initial overlaps on spawn (M1 soak blocker fix `86c9m36zh`)

- Decided by: orchestrator (with Devon)
- Decision: `Hitbox.gd::_ready` MUST defer a `_check_initial_overlaps` sweep that walks `get_overlapping_bodies()` + `get_overlapping_areas()` and calls `_try_apply_hit(target)` for each. Connecting `body_entered` / `area_entered` alone is insufficient: Godot 4 Area2D fires those signals only on entry events, so any node already overlapping the hitbox at the moment it's added to the tree is silently missed. Pre-fix, attacking a mob you were already touching did nothing. The deferred sweep is the canonical pattern; future Area2D-based combat surfaces (boss-specific hitboxes, projectile splash, charged-attack rings) MUST follow the same idiom.
- Why: Sponsor's interactive M1 soak on RC `4484196` discovered combat felt completely non-functional from the player's perspective — spam-clicking on a touching grunt produced no damage. The bug was invisible to every existing combat test because each one bypasses Godot's signal layer by calling `hitbox._try_apply_hit(target)` directly (see `tests/test_hitbox.gd`, `tests/test_player_attack.gd`, `tests/integration/test_ac3_combat_loop.gd`, `_walk_in_and_kill` in `tests/integration/test_m1_play_loop.gd`). The new paired test `tests/integration/test_hitbox_overlapping_at_spawn.gd` exercises the engine signal flow and locks the regression.
- Reversibility: reversible — the sweep is one block; if a future combat-shape change ever requires entry-only semantics it can be opt-in via a flag. The single-hit-per-target invariant (`_hit_already`) is unaffected; pre-existing-overlap + `body_entered` for the same target = exactly one hit.
- Affects: Devon (Hitbox.gd owner, paired test author); Drew (mob-spawned hitboxes — already inherit the fix via the shared Hitbox.gd code path; no per-mob change required); Tess (paired-test pattern — every future Hitbox-style test SHOULD let the engine fire signals OR explicitly document why it's bypassing them).
- Detail: PR `devon/fix-combat-hits`, ClickUp `86c9m36zh`, file `scripts/combat/Hitbox.gd::_check_initial_overlaps`.
