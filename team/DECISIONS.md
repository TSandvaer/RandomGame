...
## 2026-05-02 — Backlog expansion proposed: 8 new tickets generated to fill dispatch envelope while Sponsor OUT.

## 2026-05-03 — M1 play loop integrated into `Main.tscn` (resolves long-standing stub gap from week-1 boot scaffold). `Main.gd` now spawns Stratum1Room01 + HUD CanvasLayer + InventoryPanel + StatAllocationPanel + DescendScreen at boot; chains Room01->Room08->BossRoom via `room_cleared` signals (Room01 synthesized from last-mob-died since it has no gate); applies M1 death rule (level + equipped survive, unequipped + run-progress reset) on `Player.player_died`; persists state on quit + per-room-clear + per-level-up via Save.save_game(0); restores autoload + Player HP state on boot. `Player.gd` gains `take_damage` / `hp_current` / `hp_max` / `player_died` so enemy hitboxes deal damage and the death loop closes. Save schema unchanged at v3 (M2 owns v4). Integration test `tests/integration/test_m1_play_loop.gd` is the gating contract for Tess sign-off. Detail: PR `devon/m1-integration`, ClickUp `86c9m2jgu`.
...
## 2026-05-09 — Out-of-combat HP regen mechanic locked

- Decided by: Uma (per dispatch authority — UX/balance design, M1 RC unblocker)
- Decision: HP regenerates at 2.0 HP/sec when out-of-combat (defined as `time_since_last_damage_taken > 3.0s` AND `time_since_last_hit_landed > 3.0s`). Regen caps at HP_MAX. Any damage taken or hit landed resets the respective out-of-combat timer. All three constants (`REGEN_RATE_HP_PER_SEC`, `REGEN_DAMAGE_COOLDOWN_SECS`, `REGEN_ATTACK_COOLDOWN_SECS`) are exported tunables in `Player.gd` (or Devon's chosen home).
- Why: Sponsor M1 RC soak surfaced that the trade math (player 100 HP / grunt 5 dmg × 2-mob rooms × 8 rooms) is unsurvivable without resource accumulation. Cross-checked against `resources/mobs/grunt.tres` (confirmed `hp_base=50`, `damage_base=5`). Sponsor explicitly chose passive regen over room-transition heal — disengagement-as-strategy is the design vocabulary. At 2.0 HP/s, recovering 30 HP deficit takes 15 s (one natural inter-room breath); at 1.0 HP/s it takes 30 s (too slow, feels like a chore); at 5.0 HP/s it takes 6 s (encounters become consequence-free).
...
## 2026-05-15 — S2 boss music: unique (placeholder synthesis, deferred-M3 promotion)

- Decided by: Uma (per dispatch authority — W3-T9 S2 audio sourcing close-out, ClickUp `86c9ue23j`)
- Decision: `mus-boss-stratum2` ships as a **unique** ~60s placeholder track, NOT cross-stratum-reuse of `mus-boss-stratum1`. The placeholder is algorithmically synthesized via the same Python composer pass that produced `mus-stratum2-bgm` (same instrumentation palette — cello drone + frame drum + bronze bell — with escalated direction: minor-third interval vs the BGM's perfect fifth, driving 80 BPM frame drum vs the BGM's 50 BPM heartbeat, louder bell strikes, two low-brass swells at phase-break beats). All three S2 cues (`mus-stratum2-bgm`, `mus-boss-stratum2`, `amb-stratum2-room`) carry a **`<deferred-M3>` flag in `audio-direction.md`** for promotion to proper DAW hand-composed cues in M3.
- Why: Brief offered the choice "cross-stratum reuse for M2 RC; ship unique only if your sourcing capacity allows." My sourcing capacity in this environment is programmatic synthesis (Python+numpy+soundfile), which has near-zero marginal cost per additional cue. Cross-stratum-reuse carries a higher risk: Cinder Vaults boss against S1 cloister music registers as a mood mismatch (warm-red rust-stone biome with cool-sandstone monastic-ruin music), and Sponsor's M2 RC soak is likely to flag it as "the boss music feels like the wrong place." Unique placeholder preserves the M3-promotion option without baking the mood mismatch into the M2 RC artifact.
- Reversibility: reversible — both `mus-boss-stratum2.ogg` (placeholder) and the M3 promotion pass (hand-composed final) sit at the same file path. Replacing the placeholder is a one-file commit. The decision that's sticky is the **shape** (boss music is unique to S2, not cross-stratum), not the specific track content.
- Affects: Devon (W3-T9 wiring follow-up — same audio-bus plumbing whether placeholder or hand-composed final; load `audio/music/stratum2/mus-boss-stratum2.ogg` into the BGM bus on S2 boss room entry); Tess (acceptance probe target: "does the S2 boss music feel different from the S1 boss music? does it feel like Cinder Vaults specifically?" — subjective Sponsor-soak signal); Uma (M3 promotion ticket — file under `<deferred-M3>` audio backlog, hand-compose in DAW with cello + frame drum + bell + brass at proper fidelity); Priya (no backlog impact — fulfills W2-T9 / W3-T9 acceptance for M2 RC).
- Detail: `audio/music/stratum2/mus-boss-stratum2.ogg`, `audio/_src/composer/compose_stratum2.py` (the reproducible synthesis script), `team/uma-ux/audio-direction.md` (`<deferred-M3>` flag), ClickUp `86c9ue23j`.

## 2026-05-15 — Three process gates installed in response to M2 RC soak meta-finding

- Decided by: Priya (per ClickUp `86c9uf0w8`, citing investigate-skill consolidator output)
- Decision: Three process gates installed in one PR to close the meta-cause of all four M2 RC Sponsor-soak findings (every gate was PR-scoped, but the bugs were journey-scoped): (1) **Regression guard** added to `team/orchestrator/dispatch-template.md` "Done clause" — every dispatch names at least one test that would fail if the feature broke in a future unrelated PR. (2) **Cross-lane integration check** added to `team/GIT_PROTOCOL.md` Self-Test Report — every UX-visible PR enumerates adjacent-lane state and reports what was probed (or names it as a Sponsor-soak probe target). (3) **Milestone-gate journey probe** added to `team/TESTING_BAR.md` § "Tess" — Tess runs ONE end-to-end journey (boot → Room01 → S1 traverse → boss → loot → save → quit → reload → resume) before any RC build reaches Sponsor.
- Why: M2 RC soak (2026-05-15) returned four user-visible findings (Room 04 Shooter AI `86c9uehaq`, boss-room loot `86c9uemdg`, leather_vest unknown-id `86c9uen3z`, squares-fighting-squares `86c9uepzm`) on a build that had cleared every per-PR gate cleanly. The investigate-skill consolidator identified the META-CAUSE: PR-scoped gates can verify components but cannot verify the journey; user-visible bugs are journey-scoped.
- Reversibility: reversible — process gates can be loosened if they over-load the team. The Tess journey-probe ~15 min cost replaces ad-hoc bug-bash time at RC, not additional.
- Affects: all roles (dispatch + Self-Test Report); Tess (RC journey-probe execution); orchestrator (enforcement on every dispatch).
- Detail: this PR; ClickUp `86c9uf0w8`; cross-references `86c9uf0mm` (universal console-warning zero-gate, which the journey-probe references for the `push_warning` blocker rule).

## 2026-05-15 — M2 W3 mid-retro captures Sponsor soak meta-findings; 5 M3 watchlist tickets parked

- Decided by: Priya (per dispatch authority — `86c9uf81y` mid-W3 retro + M3 watchlist filing)
- Decision: New doc `team/priya-pl/m2-week-3-mid-retro.md` (~430 lines, 11 sections) captures the Sponsor M2 RC soak meta-findings (4 direct + 2 also-found + 1 latent bug class), the investigate-skill consolidator output (3 harness investments + 3 process gates), Drew's `latestPos` stale-trace finding, the diagnosis-via-trace discipline hardening (PR #200 → PR #212 pattern), the risk-register update (R6 FIRED, R2 strained, R11 held-but-evolved into PR-scoped vs journey-scoped meta-pattern), pending work + W4/M3 deferrals, and the mid-W3 grade (B+). Five M3 watchlist bug classes filed as parked ClickUp tickets (status to do, priority low, tags m3+qa): visual-fidelity Tier-2 snapshot baseline, audio HTML5 Web Audio decoder smoke, tab-blur browser-lifecycle probe, RNG drop-rate distribution, mob-side AI-state trace assertions.
- Why: W3 mid-week is an inflection point, not a close — Sponsor's first M2 RC soak produced exceptional density of findings + the team's institutional response (PR #216's three process gates) is unprecedented in cycle-time. Capturing while institutional memory is fresh prevents the meta-findings from being lost between M2 RC sign-off and M3 kickoff. The 5 M3 watchlist tickets exist as tracker rows, not active work.
- Reversibility: sticky — institutional-memory document. The risk-register updates (R6/R2/R11) ride on the existing rolling-doc pattern; the 5 M3 watchlist tickets are parked and explicitly out-of-scope for W3/W4.
- Affects: orchestrator (audit next 3-5 dispatch briefs for adoption of PR #216's Regression-guard + Cross-lane integration check lines; ensure next M2 RC handoff carries a Tess journey-probe artifact); Tess (5-item queue + new RC-boundary journey-probe gate per PR #216); Drew (Room 04 Shooter AI fix + Room 05 player-death recurrence drain — gates AC4 white-whale flip); Priya (next risk-register refresh documents R11's evolved PR-scoped vs journey-scoped framing).
- Detail: `team/priya-pl/m2-week-3-mid-retro.md`, 5 M3 watchlist ticket IDs in this PR's body, ClickUp `86c9uf81y`.

## 2026-05-15 — Vault-Forged Stoker visual direction: deepest-mine-seam sub-biome + ColorRect breath-cone primitive

- Decided by: Uma (per dispatch authority — W3-T4 prep, ClickUp `86c9uf86n`)
- Decision: The S2 boss (Vault-Forged Stoker) arena ships in a **deepest-mine-seam sub-biome variant** of the Cinder Vaults palette — two value-shifts darker on floor/wall base hexes, 80% wall-vein density (vs 30% standard), vignette 55% (vs S2's 40%), ambient tint 12% alpha (vs S2's 8%); implementable as a chunk-level CanvasModulate retint, NOT new sprite authoring. The breath-cone telegraph uses a **rotated `ColorRect` primitive** (NOT Polygon2D) at 60° base cone / 120 px range / 1.2s telegraph windup + 0.8s commit, mirroring the M1 swing_wedge primitive after PR #137 to preempt the WebGL2 HDR-clamp + Polygon2D divergence. Phase-3 enrage widens the cone to 90° / 150 px / 0.9s windup with audio-intersection at `mus-boss-stratum2`'s first low-brass swell apex (T+0.8 from enrage trigger). Boss intro reuses `boss-intro.md` Beat 1-5 verbatim with one Stoker override on Beat-2 (wall veins FLARE instead of S1's torch-dim — diegetic load for "veins recognize the boss"). Body-vein motif on the boss sprite reuses the wall-vein palette `#C25A1F`/`#FF6A2A` — the same substance as the player's flame, setting up S7-S8 fiction escalation.
- Why: (a) Sub-biome mirrors S1 boss precedent (Stratum1Boss arena is a darker corner of Cloister palette) and reinforces the unique-music decision from earlier today by visually distinguishing the boss arena from the rest of S2 — `palette-stratum-2.md` §8 q1 explicitly delegated this call to Uma+Drew. (b) ColorRect breath-cone is the html5-visual-verification-gate.md memory rule applied preemptively; Polygon2D for cones is the natural desktop-Godot choice that would replicate the PR #115/#122 cautionary tale. (c) Body-vein motif resolves the visual-vs-audio reinforcement requirement from the unique-music decision — without it the boss reads as "bigger Stoker" not "vault-forged champion."
- Reversibility: reversible — sub-biome is a single CanvasModulate alpha + chunk TRES override; breath-cone primitive choice is a Drew implementation decision binding only the visual primitive, not the cone geometry semantics. The sticky elements are the diegetic body-vein motif (S7-S8 fiction load) and the visual-audio intersection point at phase-3 enrage (Devon's audio wiring depends on the same `phase_changed.emit(3)` signal that fires the visual brightness-spike).
- Affects: Drew (W3-T4 state machine + scene authoring — the doc IS the dispatch brief: §1 sprite anchors, §2 cone primitive + colors + particle budget, §3 phase-transition states preserving Stratum1Boss three-case rejection trace, §4 intro Beat-2 vein-flare override + Beat-3 timing +0.1s, §5 T3 weapon loot tint); Devon (Beat-5 boss music handoff = `mus-boss-stratum2` not `mus-boss-stratum1` + phase-3 enrage audio crescendo intersection at T+0.8 from enrage trigger); Tess (HTML5 visual-verification gate applies — paired test mirrors `test_player_swing_flash_tint_is_html5_safe`; stagger-immune rejection trace mirror Stratum1Boss `IGNORED phase_transition`; Beat-F screenshot/video required); Uma (no follow-up — design layer closes here); Priya (T3 affix-pool follow-up filed as Open question 2 in `team/uma-ux/vault-forged-stoker.md`, non-blocking).
- Detail: `team/uma-ux/vault-forged-stoker.md` (the full synthesis-for-Drew doc), ClickUp `86c9uf86n`.

## 2026-05-15 — M3 design-seeds doc to include character-art-pass §4 (Sponsor-promoted)

- Decided by: Sponsor (via orchestrator dispatch)
- Decision: `team/priya-pl/m3-design-seeds.md` (W3-T12) expands from three sections (multi-character, hub-town, persistent meta-progression) to four sections — adds §4 character-art-pass milestone framing, Sponsor-promoted from M2 RC soak ("when is it going to change from squares fighting squares to actual graphic?").
- Why: Sponsor surfaced the placeholder-art question during M2 RC soak; promotes the topic from "M3 someday" to a Sponsor-visible roadmap requirement. Sponsor explicitly approved Option 1 (Priya scopes character-art-pass into the M3 design doc; Sponsor gets a costed roadmap before committing).
- Reversibility: reversible — design seeds, not design lock.
- Affects: Priya (this doc); Uma (M3 hub-town visual + NPC sprite direction); Drew (M3 stratum-by-stratum sprite authoring track); Devon (v5 save-schema migration §1 + §3 implications); Sponsor (6 big-shape sign-off items + external cost-bracket estimate commission).
- Detail: `team/priya-pl/m3-design-seeds.md` (this PR).

## 2026-05-18 — Stoker brief contradiction resolved: M3 phase-1 ships as Grunt-retint; miner-cap re-authoring deferred to phase 2

- Decided by: Uma + Priya joint reconciliation (orchestrator-dispatched parallel agents, both independently converged on Option C — Hybrid).
- Decision: M3 phase-1 ships **Stoker as a palette-retint of Grunt v2** (cloth `#7A1F12`, skin `#7E5A40`, aggro eye-glow `#D24A3C`) — already realised by PR #263 (idle sprite) and PR #265 (uses Grunt v2 animation set at runtime). The original `palette-stratum-2.md` §5 doctrine — **NEW AUTHORING with miner's-cap silhouette** — is preserved as the phase-2 target. A backlog ticket re-authoring Stoker with the miner silhouette is scheduled to land alongside or after S2 boss-room work (M4 timing TBD).
- Why: Ship-state already wins de-facto (Stoker idle + animations merged on `main`), and Devon/Drew scene-wiring is next. Option A (ship-state silently overrides §5) hides the doctrinal debt; Option B (revert + author fresh) burns ~30-50 PixelLab gens + cleanup pass and blocks Devon/Drew on the art-pass. Option C honors shipped reality + preserves Uma's doctrine as an explicit target. Uma's visual reasoning: the hooded-monk silhouette is salvageable in S2 — fiction-framed as "a cloister-novice who descended too deep," with palette + sub-biome context carrying the tonal load until phase-2 re-author lands. The body-vein motif from `team/uma-ux/vault-forged-stoker.md` is scoped to the BOSS not the grunt-tier Stoker, so the silhouette gap is second-order for the tier-1 mob. Priya's scope reasoning: PixelLab budget (~303/2000 on Tier 1) is not the constraint — coherence is — and paying it once explicitly via a phase-2 ticket is cleaner than paying twice (now + on re-author).
- Reversibility: reversible — if M3 soak surfaces "Stoker reads as hooded monk in S2" findings ≥2 times, escalate to phase-2 re-authoring earlier (still no code revert; just an earlier PixelLab pass). Sticky element is the phase-2 ticket; once filed, it carries the debt forward across M4 planning.
- Affects: Uma (`palette-stratum-2.md` §5 Grunt-row + summary updated this PR to reflect dual M3/phase-2 framing); Priya (`art-pass-ai-primary-brief.md` Stoker references annotated this PR with Phase-1 framing; M3 design-seeds unchanged); Drew/Devon (scene-wiring proceeds on the Grunt-retint Stoker — no waiting on art-pass); Tess (M3 RC soak watches for "Stoker reads as hooded monk in S2" findings — would escalate phase-2 timing); Sponsor (no action; informed via PR description).
- Detail: this PR (`docs/stoker-contradiction-resolution`) edits `team/uma-ux/palette-stratum-2.md` §5 + `team/priya-pl/art-pass-ai-primary-brief.md` Stoker references; ClickUp ticket [`86c9uze5j`](https://app.clickup.com/t/86c9uze5j) "feat(art): re-author Stoker with miner-cap silhouette (phase 2)" filed in RandomGame list with `m4` + `phase-2` + `stoker` tags, priority low.

## 2026-05-18 — Stoker runtime palette-swap mechanism: Path A (pre-bake separate atlas via pixel-mcp doctrine-lock)

- Decided by: Uma + Devon joint reconciliation (orchestrator-dispatched parallel agents, both independently converged on Path A).
- Decision: Stoker's M3-phase-1 animation set will be **pre-baked as a separate Stoker atlas at `assets/sprites/stoker/_pixellab_anims_baked/`** by running the Grunt v2 animation frames through `pixel-mcp` doctrine-lock with the S2 mob palette (cloth `#7A1F12`, skin `#7E5A40`, aggro eye-glow `#D24A3C`) per the per-slot nearest-neighbor "BEST" strategy documented in `.claude/docs/pixel-mcp-pipeline.md`. Devon will author a separate Stoker `SpriteFrames` `.tres` referencing the baked atlas; Drew wires the Stoker scene to point at it. **Rejected:** shader-based palette remap at draw-time (Path B) due to WebGL2 `gl_compatibility` shader-divergence risk per `.claude/docs/html5-export.md` + `[[html5-visual-verification-gate]]` memory (PR #115/#122 precedent). **Rejected:** Godot `modulate` two-tone (Path C) because multi-channel Grunt source doesn't survive single-color multiplication.
- Why: (Uma, visual-doctrine lens) S2 mob palette is hex-exact not tolerance-banded — §2 mob table specifies the locks; S2-PL-05 is an eye-dropper QA check; shader edge-interpolation would seed non-doctrine intermediates and re-open the cross-stratum aggro-eye contract (PL-11). Pre-bake is the only path that guarantees every pixel lands on the locked palette. (Devon, runtime/HTML5 lens) Zero runtime risk; identical SpriteFrames lookup path to Grunt; +2.5MB disk cost is trivial vs. WebGL2 shader gamble on a Sponsor-soak surface. Reuses the already-validated authoring pipeline rather than introducing a parallel shader-debug + WebGL2-verification surface that would compound across S3-S8 mob retints later (~6 retints).
- Reversibility: reversible — re-bake batch is ~5 min if doctrine hex shifts later. Sticky element is the **cross-stratum retint pattern** — this decision establishes pre-bake-via-pixel-mcp as the canonical mob-retint pipeline for S3-S8 work as well; switching to a shader path later would require re-doing all retints under the new pipeline.
- Trap to watch (per Devon): `pixel-mcp.fill_area tolerance=0` is global-replace, NOT flood-fill — if any Grunt source frame has overlapping antialias hexes between cloth and skin regions, naive positional `set_palette` will fail. Use per-slot nearest-neighbor mapping (Charger-validated BEST strategy per `.claude/docs/pixel-mcp-pipeline.md`).
- Open question (per Uma): if eye-glow `#D24A3C` reads too close to S1 ember accents at silhouette-distance after the swap, Uma may demand a minor outline/shadow re-tone beyond pure palette remap. Bake validation passes through HTML5 visual-verification gate; Uma signs off on the first Stoker bake before downstream PRs inherit the pattern.
- Affects: Devon (Stoker SpriteFrames `.tres` authoring + pixel-mcp doctrine-lock batch run for the 5 Stoker anims — depends on Grunt v2 atlas being wired in M3W-3 first per `team/priya-pl/m3-scene-wiring-scope.md`); Drew (Stoker scene wiring at M3W-5 references the baked atlas); Uma (silhouette-read-at-distance verification on first Stoker bake before downstream inheritance); Tess (HTML5 visual-verification gate on the M3W-5 PR; eye-dropper QA against `palette-stratum-2.md` §2 hexes); Sponsor (no action; informed via PR description).
