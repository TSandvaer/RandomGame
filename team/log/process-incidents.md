...
## 2026-05-22 — Morning-investigation incomplete-verification + cwd-inheritance on P0 `86c9xw8xd` chase (correct-for-Wave-1, stale-for-current-HEAD)

- Filed by: orchestrator (this session)
- Severity: medium (caused ~1 day of misdirected work — Track A dispatch was a wasted cycle; recovered by afternoon Tess refutation + Devon investigation; ~2 agent cycles refunded out of a 7-day-old P0 saga)
- Repeat?: first occurrence (logged for pattern-watch + structural mitigation landed same session)
- What happened: P0 ticket `86c9xw8xd` (Playwright-red on `main` for 7 days since 2026-05-15) opened. Tess's morning investigation identified PR #215's `mob-self-engagement.spec.ts` `team=mob target=Player` regex as the original-cause-of-record + wrote a 220-line `team/tess-qa/playwright-red-main-investigation-2026-05-22.md`. **The morning finding was REAL for the original Wave-1 cause** (PR #215 era through 2026-05-16). Orchestrator dispatched Tess on a Track A authoring brief based on this investigation. **Afternoon Tess (fresh dispatch, same role) refused authorship** per the `agent-verify-evidence` discipline, with cite-able evidence that: (a) the current spec regex at HEAD is `team=enemy` — fixed PR #249 commit `a885d56` 2026-05-16 = 6 days BEFORE the morning investigation; morning Tess identified the correct historical pattern but did NOT verify it persisted in current HEAD; (b) Playwright `test.fail()` blocks rendering as `✘` glyphs in text output were contributing to the apparent failure count — those count as PASS in the `N passed` summary line; (c) the actual `1 failed` on recent main runs was a different spec entirely (`equip-flow.spec.ts:142`). Devon's subsequent investigation revealed BOTH morning + afternoon were correct in their RESPECTIVE windows: Wave 1 (2026-05-15 → PR #249 merge 2026-05-16) was mob-self-engagement (morning's finding); Wave 2 (2026-05-17 onward) was equip-flow line-slice anchor regression (afternoon's finding). **Morning Tess did not fabricate** — she identified a genuine historical root cause; the failure mode was **incomplete verification** (failed to check whether the regex was still in that shape at current HEAD). **Adjacent infrastructure failure:** the morning investigation doc was written in the ORCHESTRATOR's ROOT worktree (`c:\Trunk\PRIVATE\RandomGame`), NOT in `RandomGame-tess-wt` where her dispatch brief targeted — same cwd-inheritance failure mode as Devon's W1 camera-scroll spike WIP-in-root on the SAME DAY. The Step-0 cd convention was codified later same session to address this exact failure (PR #318 merged `3b77050`); morning Tess's dispatch predated the Step-0-cd codification.
- Why it didn't cause a defect: ~1 day of P0-closure delay + ~30 min Track A dispatch sunk before refusal-to-author. No code regressions. P0 ultimately closed cleanly on 2026-05-22 15:24 UTC after PR #317 merge + main release-build manual trigger + chain verification (equip-flow:142 GREEN on run `26294689527`). Mitigation 1 (orch merge-gate verification) landed in PR #316 the same day and was structurally correct regardless of which wave was diagnosed.
- Protocol clarification: four conventions codified across PRs #316, #318, and #321 this same session as direct response to the incident pattern:
  - **Bisect-by-spec-name, not by overall workflow conclusion** — when CI workflow `conclusion=failure` over a window, bisect to identify which spec(s) are the failure source; the same workflow may stay red continuously while the underlying spec changes underneath. Codified in `.claude/docs/test-conventions.md` § "Control-comparison technique" (PR #321).
  - **`test.fail()` glyph triage discipline** — Playwright renders `test.fail()` blocks that throw a placeholder error as `✘ [<spec>]` in text output, but the `N failed` summary counts them as PASS. Bisect by spec-name from the `N failed` summary line, NOT by counting `✘` glyphs. Codified in same PR #321 section.
  - **Cite-able evidence in final reports** — `tightened-final-report-contract` amendment (PR #316) now requires every "X is green" claim cite verifiable evidence (PR # / commit SHA / run-id URL / file:line). Morning Tess's report cited the historical regex shape without independently verifying it persisted in current HEAD; the cite-able-evidence contract would have forced a "regex at HEAD `a885d56:tests/playwright/specs/mob-self-engagement.spec.ts:144`" line that exposes the staleness immediately. Codified in `tightened-final-report-contract` memory + dispatch-template.
  - **Step-0 `cd <worktree>` convention** — sub-agent shell tools inherit orchestrator cwd, NOT the worktree path the brief names. Naming the worktree is insufficient. Codified in `.claude/docs/orchestration-overview.md` § Dispatch conventions (PR #318). Morning Tess's dispatch predated this convention; the investigation doc landed in root and was discovered there during the same-session cleanup pass.
- Cross-references: P0 `86c9xw8xd` (the saga ticket, now `complete`); Track A `86c9xwfwg` (closed NOT_A_BUG); Track C `86c9xwfyr` (closed gated-on-invalid-parent); Track A.NEW `86c9xwu50` (closed via PR #317 fix); morning investigation doc path: `team/tess-qa/playwright-red-main-investigation-2026-05-22.md` (never committed); PR #316 (Mitigation 1 + cite-able-evidence amendment); PR #321 (control-comparison + `test.fail()` glyph + rebase-re-triggers-CI captures); `agent-verify-evidence` memory (the discipline that caught the misread on second pass).
- Action taken: log + structural conventions landed in PR #316 and PR #321 (same session). No individual blame — pattern was a structural gap (cite-able-evidence contract did NOT yet exist when morning investigation ran; that gap is now closed). Pattern-watch: if a 2nd misread-by-glyph or lost-investigation-doc incident lands AFTER the conventions are in place (post-PR #321 merge), escalate to a stricter pre-dispatch verification step in the dispatch-template (orchestrator MUST verify investigation cites against current HEAD before authorizing follow-up work).

---

## 2026-05-31 — Integ-branch PRs get NO automated CI (ci.yml main-only trigger)

- Filed by: Priya (run, W3-T7 Stage-6 hygiene; surfaced by Tess during #385 QA)
- Severity: medium (no defect yet, but a blind spot in the safety net during a multi-round soak-fix convergence)
- Repeat?: first occurrence (logged for pattern-watch + re-soak-#5 merge-plan guard)
- What happened: W3-T7 Stage-6 (S2 ArchiveSentinel boss room, ticket `86c9y7ygj`) is converging 4 soak-fix rounds onto an integration branch `integ/stage6-resoak4` for Sponsor re-soak #5. `.github/workflows/ci.yml` triggers only on `pull_request: branches: [main]`, so any PR based on `integ/stage6-resoak4` (e.g. #385 death-zoom fix, base=integ) gets NO automated CI run — GitHub never registers the workflow for a non-`main`-targeted PR. Agents compensated by running GUT locally and citing the local results in Self-Test Reports / QA notes. Tess flagged the gap during #385 QA.
- Why it didn't cause a defect: agents ran GUT locally per PR and Tess QA'd each fix on the integ HEAD; the integ branch is an assembly convenience, not a merge target to main. The real CI gate still fires when each fix rebases onto main at the final Stage-6 merge.
- Protocol clarification: the integ-branch pattern is sound for assembling a single Sponsor soak build, but it silently disables the automated CI gate for stacked PRs. **Guard for the re-soak-#5 merge plan:** when the integ chain rebases onto main for the final merge, CI MUST fire and be confirmed green PER PR post-rebase-to-main before flipping `86c9y7ygj` complete — local-GUT-on-integ is necessary-but-not-sufficient; the main-targeted CI run is the binding gate. This mirrors the existing stacked-base-CI caveat already noted in STATE.md for the #376→#377→#380 boss chain (CI does not auto-fire on stacked/non-main bases; confirm green each merge step).
- Cross-references: ticket `86c9y7ygj` (Stage-6 parent); integ branch `integ/stage6-resoak4`; PR #385 (death-zoom fix, base=integ); PR #386 (RMB contextmenu, base=main — fires CI normally, contrast case); `.github/workflows/ci.yml` `pull_request: branches: [main]` trigger; STATE.md Resume header "PROCESS GAP (Tess flag)" + stacked-base-CI caveat.
- Action taken: log + re-soak-#5 merge-plan guard noted (confirm green main-targeted CI per PR post-rebase). Pattern-watch: if a future milestone routinely uses integ branches for multi-round soak convergence, escalate to a `ci.yml` trigger addition (e.g. `branches: [main, 'integ/**']`) so integ-based PRs get CI automatically — weigh against extra CI minutes.

---

## 2026-05-31 — In-PR behavior retune left that PR's own earlier test assertions stale (#380 cadence)

- Filed by: Priya (run, W3-T7 Stage-6 hygiene; surfaced during #380 cadence retune)
- Severity: low (caught pre-merge; Drew refreshing the assertions before #380 lands green)
- Repeat?: first occurrence (logged for pattern-watch)
- What happened: PR #380 (phase-blink + cadence) received an in-PR behavior retune (commit `2388e7e`: BLINK_FLOOR P1 2.5→5.0s / P2 1.5→3.5s + defensive-range + per-blink volley changes) after the Sponsor re-soak #2 finding ("boss moves around too much"). The retune left TWO of #380's OWN earlier test assertions stale: (1) GUT `test_cast_bolt_spawns_at_book_travels_to_captured_target` and (2) Playwright `stratum2-boss-room.spec.ts` step-4e blink assertion (encoded the old 20s/passive-far-player cadence vs the new 5s-floor + defensive-range behavior). Drew is refreshing both on `drew/sentinel-invisible-attack-fix` before #380 merges green.
- Why it didn't cause a defect: caught before merge; the stale assertions surfaced as RED in the pre-merge test pass, which is exactly when they should. No shipped regression.
- Protocol clarification: the class is **"a behavior-retune within an open PR can leave that PR's OWN earlier test assertions stale."** The reflex to re-run the full test set tends to fire after upstream/base changes, not after an author's own in-PR tuning commit — but a tuning commit that changes observable behavior is exactly as likely to invalidate earlier-in-the-same-PR assertions. **Convention:** after any in-PR behavior-changing commit (constant retune, cadence change, threshold move), re-run the PR's FULL test set (not just the new tests added for the change) before re-requesting review — earlier assertions in the same PR may now encode the pre-retune behavior.
- Cross-references: PR #380 (`drew/sentinel-invisible-attack-fix`); retune commit `2388e7e`; ticket `86c9y7ygj` (Stage-6 parent); STATE.md "PRE-MERGE CLEANUP" entry (the 2 stale contracts). Adjacent class: `spec-string-vs-engine-emit drift` (test assertion encodes a stale contract) — but distinct: that's emit-vs-spec drift; this is author-retune-vs-own-prior-assertion drift.
- Action taken: log + convention noted (full-test-set re-run after in-PR behavior change). Pattern-watch: if a 2nd in-PR-retune-leaves-own-tests-stale incident lands, escalate to a dispatch-template line reminding authors to re-run the full PR test set after any behavior-changing tuning commit, not just the new tests.
