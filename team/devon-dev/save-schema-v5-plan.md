...
- **New top-level keys under `data{}`:** `characters: Array` (replaces `character`), `active_slot: int` (replaces "implicit single-character" semantics), `shared_stash: Array` (replaces `character.stash`). No new `meta` fields in v5 (the first-visit `hub_town_seen` gate lives per-character per Sponsor 2026-05-17 decision; see §2.3).
- **New per-character keys under `data.characters[N]`:** `slot_index: int`, `ember_shards: int`, `active_bounty: Dictionary | null`, `paragon_points: int`, `paragon_spent: Dictionary`, `hub_town_seen: bool` (per-character per Sponsor 2026-05-17 — reversed from this spike's original `meta.hub_town_seen` account-scoped default), `hub_town_last_descended_stratum: int` (additive within the v5 multi-character structure; the latter two per Uma's `hub-town-direction.md §7` cross-reference).
...
- **Open questions for Sponsor/Priya/Uma/orchestrator:** 5 (catalogued in §9). The Devon-call (pointer-shadow) is decided in this doc (§4.4 — YES). Q-6 (`hub_town_seen` scope) was decided by Sponsor 2026-05-17 — **per-character**; the spike has been amended in-flight (field moved from `meta` to `data.characters[N]`); §9 retains the entry for the audit trail.
...
| `data.characters[N].hub_town_seen` | bool | `false` | Y | First-visit hint-strip gate per `m3-design-seeds.md §2 Save-schema implications` + Uma's `hub-town-direction.md §7`. **PER-CHARACTER per Sponsor 2026-05-17 decision** (reversed from this spike's original account-scoped default at `meta.hub_town_seen`). Each new character sees their own first-visit welcome beat, diegetic-first framing per Uma — "each new character is encountering the cloister for the first time as themselves." Aligns with Shape A's multi-character pillar intent (per-character state is the M3 model). Trade-off: slight save-schema complexity (one extra per-character bool) vs simpler account-scoped; cost is negligible. Migrated v4 characters initialize to `false` (hint fires on first hub-town entry, same as fresh M3 characters). |
...
That's the v5 delta. **One non-additive structural change** (character: Dict → characters: Array) plus three path-lifts (equipped, stash, ember-bags) plus **eight** purely additive fields (`slot_index`, `active_slot`, `shared_stash`, `ember_shards`, `active_bounty`, `paragon_points`, `paragon_spent`, `hub_town_seen`, `hub_town_last_descended_stratum` — nine additive fields total counting the new top-level `characters` Array container itself; "eight purely additive" excludes the structural-substitution fields). All new bool/int/Array/Dict additions are per-character except `active_slot` (top-level int) and `shared_stash` (top-level Array); no new `meta` fields.
  ...
  characters: [                              # NEW (replaces `character`; type-changed)
    {
      ...
      hub_town_seen: bool                    # NEW (per-character, Sponsor 2026-05-17)
...
function migrate_v4_to_v5(data: Dictionary) -> Dictionary:
    ...
    if not existing_character.has("hub_town_seen"):
        existing_character["hub_town_seen"] = false                 # PER-CHARACTER per Sponsor 2026-05-17
    ...
    # --- Step 4: Ensure meta exists (no new meta fields in v5 per Sponsor 2026-05-17;
    # hub_town_seen lives per-character now, not on meta). Defensive backfill only. ---
...
- **No change to `meta.runs_completed`, `meta.deepest_stratum`, `meta.total_playtime_sec`.** These remain on `meta` (per v4 plan §3.2 — meta is account-scoped). **No new `meta` fields in v5** (the first-visit `hub_town_seen` gate lives per-character per Sponsor 2026-05-17 decision; see §2.3).
...
1. **Extends v4's compat-shadow doctrine with stricter discipline.** v4 already keeps `data["stash"]` (v1 stub) alongside `character.stash` per v4 plan §3.2; v3 keeps flat `vigor/focus/edge` alongside `stats.{vigor,focus,edge}` per `_migrate_v2_to_v3`. The project's pattern for "rename without breaking" is the compat shadow. **v5 tightens the contract:** v3/v4 shadows are write-once-on-migration (the migration emits both shapes; subsequent saves let them drift if any code path mutates one without the other). v5's `data.character` → `data.characters[active_slot]` shadow is **active-refresh on every `save_game`** (per INV-5), making the discipline test-pinned rather than convention-pinned. v5 extends the family, but with a stricter freshness guarantee than its predecessors.
...
4. **The drift discipline is enforceable in tests.** INV-5 (§5) pins `data.character == data.characters[active_slot]` at every save. If a v5 runtime path mutates `data.character` directly (the failure mode this discipline guards against), the test fails immediately. The shadow's read-only contract is enforced empirically.
...
- **"Pointer-shadow risks drift."** Mitigated by INV-5 (drift detector) + the read-only convention. The same risk exists for `data.stash` ↔ `character.stash` in v4; v4 plan §3.2 acknowledges this and accepts it as the cost of compat. v5 is in-pattern.
...
| **INV-6** | `v5_new_field_defaults` | After loading a v4 fixture, `loaded["characters"][0]["ember_shards"] == 0`, `loaded["characters"][0]["active_bounty"] == null`, `loaded["characters"][0]["paragon_points"] == 0`, `loaded["characters"][0]["paragon_spent"] == {}`, `loaded["characters"][0]["hub_town_seen"] == false` *(PER-CHARACTER per Sponsor 2026-05-17)*, `loaded["characters"][0]["hub_town_last_descended_stratum"] == 1`. `meta` block is unchanged from v4 (no new meta fields in v5; `hub_town_seen` migrated out of meta per Sponsor decision). |
...
| Per-character new fields (ember_shards, active_bounty, paragon_points, paragon_spent, hub_town_seen, hub_town_last_descended_stratum) × N slots | n/a | ~240 B per character × 3 = ~720 B | +720 B |
...
2. **Hub-town first-visit** — `data.characters[active_slot].hub_town_seen = true` is set on first entry (per-character per Sponsor 2026-05-17). Must trigger `Save.save_game()` synchronously so a process-kill before next save doesn't replay the first-visit hint for that character. **One-shot per character** (each character has their own flag; new characters re-see the hint). Trivial frequency change.
...
- Decision: Save schema v5 spec'd at `team/devon-dev/save-schema-v5-plan.md` — first non-additive schema bump in project history. `data.character` (Dict) -> `data.characters` (Array of Dict); `data.equipped` lifts to per-character; `data.character.stash` lifts to `data.shared_stash`. **Eight new additive per-character fields** (slot_index, ember_shards, active_bounty, paragon_points, paragon_spent, hub_town_seen *(per Sponsor 2026-05-17 — per-character not account-scoped)*, hub_town_last_descended_stratum) + two new top-level (active_slot, shared_stash). **No new meta fields** in v5 (`hub_town_seen` lives per-character). Pointer-shadow `data.character` + `data.equipped` for one schema generation; deprecate at v6. Eight round-trip invariants (INV-1..INV-8) pinned for Tess's M3 Tier 2 migration test, including INV-5 (pointer-shadow drift detector) and INV-8 (v0->v5 chain). Eight test fixtures catalogued. HTML5/OPFS size delta +2-4 KB typical, well within quota; no chunking needed. Updated rule 6 added to v4 plan's additive-only doctrine ("non-additive permitted when no additive expression exists; must pointer-shadow + document + schedule deprecation"). Implementation lands in M3 Tier 2.
- Why: Multi-character is fundamentally non-additive (the "one character per save" assumption is baked into the type of `data.character`). One clean break + pointer-shadow + documented deprecation > decades of v4-shaped contortions. Pointer-shadow doctrine extends v4's existing stash/vigor/focus/edge compat-shadow family, but with stricter active-refresh discipline (INV-5) — v3/v4 shadows are write-once-on-migration; v5's are refreshed every `save_game`.
...
6. **`hub_town_seen` scope — account vs per-character. RESOLVED (Sponsor 2026-05-17): PER-CHARACTER.** This spike's original v1 default was account-scoped on `meta.hub_town_seen` (rationale: "once any character has seen the hub-town hint, subsequent characters skip it" — same player at the keyboard, hint utility expires after first viewing). Uma's `hub-town-direction.md §7` (PR #258) framed it as per-save-slot diegetic-first. Sponsor reversed to **per-character** mid-amendment; rationale: each new character sees their own first-visit welcome beat; aligns with Shape A's multi-character pillar intent (per-character state is the M3 model); trade-off (slight save-schema complexity vs simpler account-scoped) is negligible. **The amendment was applied in-flight in this doc** — field is now `data.characters[N].hub_town_seen: bool` (default `false`); migration function backfills it per-character; INV-6 updated; §2.4 compact schema reflects the move. Entry retained for audit trail per `decisions-while-away.md` convention; Priya batches into `team/DECISIONS.md` on weekly rollup. **Status:** resolved; no further action.
...
- **Uma (no immediate action):** the per-character `hub_town_seen` first-visit gate is the only audio/visual-direction-adjacent v5 field (per Sponsor 2026-05-17 decision; lives under `data.characters[N].hub_town_seen`, not on `meta`). Confirm at hub-town visual-direction PR (`86c9uth6a`).
...
2. **The pointer-shadow doctrine is the project's "soft-rename" pattern — and v5 tightens it.** v3 keeps flat `vigor/focus/edge` alongside `stats.{vigor,focus,edge}`; v4 keeps `data.stash` (root) alongside `character.stash`; v5 extends the pattern to `data.character` ↔ `data.characters[active_slot]`. The pattern is "rename without breaking" — the new canonical lives next to the old shadow for one schema generation. **Crucially, v5's shadow contract is stricter than v3/v4's:** v3/v4 shadows are write-once-on-migration (the migration emits both shapes; subsequent saves rely on convention to keep them in sync), while v5's shadow is **actively refreshed on every `save_game`** with INV-5 as the empirical drift detector. Recognizing this as the project's emergent doctrine — and that v5 raises the freshness/test-pinning bar — is the most useful framing for future schema-bump authors.
...
6. **M3 Tier 2 hub-town first-visit save timing — HTML5 hitch risk.** Per Drew's PR #256 review: setting `data.characters[active_slot].hub_town_seen = true` on the first hub-town entry triggers a `save_game` call, which under v5's active-refresh discipline (INV-5) also rebuilds the `data.character` + `data.equipped` shadows. On low-end HTML5, that synchronous serialize + IndexedDB write is a ~50-100ms hitch. If the hub-town first-visit hint dismisses via a "press any key" gate, the M3 Tier 2 implementer should **schedule the per-character `hub_town_seen` flip + save AFTER the hint dismisses**, not mid-door-cross. Implementation pointer for whoever picks up the M3 Tier 2 hub-town ticket; the schema contract itself is unaffected.
