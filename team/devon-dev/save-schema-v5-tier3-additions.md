...
> **Footnote (W2-T4, ticket `86c9y108t`, 2026-05-23) — Save.gd HEAD was v4 at survey authorship time.** When this survey was authored (PR #320, Drew nit 3), `Save.gd::SCHEMA_VERSION = 4` and the v5 multi-character lift catalogued in `save-schema-v5-plan.md` was paper-only. Forward-looking helper names used in this survey (e.g. `_migrate_v5_to_v5_tier3` at §5) reflect "the migration step that would land after v5 ships" — they do not map to a real function at survey-authorship time. **As of W2-T4 (2026-05-23) `Save.gd::SCHEMA_VERSION = 5`**, but the bump shipped via `_migrate_v4_to_v5` for the `world_seed` canonical promotion ONLY — the v5 multi-character lift (`data.character` → `data.characters[]`, equipped lift, shared_stash lift, `active_slot`) remains paper-only and will land in a separate downstream ticket. The forward-looking names in this survey continue to apply to whatever future migration helper lands the per-character keys; the W2/W3 impl tickets enumerated in §8 add their fields under the same `data.character` (single, current) or `data.characters[N]` (future multi-character) namespace, as appropriate to whichever lift status is live at impl time.
