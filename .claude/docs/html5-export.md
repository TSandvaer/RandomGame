...
- **Flat multi-material floor seam — paint-not-stack to eliminate the same-z tie-break (PR #426 AC9, 2026-06-07).** When two ground materials meet on a FLAT floor (e.g. a warm slab path threading cool cobble), do NOT layer them at different `z_index` — the seam is a flat ground transition, not an overlap, so stacked-z just re-introduces the same-z draw-order divergence class above. Instead paint material B into its OWN `TileMapLayer` at the SAME `z_index = 0` **and erase the underlying cell in layer A** so exactly ONE tile-class renders per cell. With no cell ever holding two tiles, there is no draw-order tie-break to diverge between `gl_compatibility` and desktop — the z-fight class is structurally eliminated, not merely ordered around. This is the correct pattern for the highest-risk camera-scroll z-fight class (multi-material floor seams across the wide viewport, `camera-scroll.md`), and generalizes to any future multi-material floor (S2+ biomes). Pinned by `test_slab_paths_painted_with_cobble_erased_beneath_no_zfight` (PR #426).
...
**When even a FRESH PORT shows the stale SHA — it's a wrong local FOLDER, not the artifact, and not the service worker (orchestrator-serves-verified-build mitigation, 2026-06-07).** Worked example: Sponsor soaked PR #424, BuildInfo read `7ddae2d` (old) instead of `49129d3` (new) across the normal cache ritual AND a brand-new port (`localhost:8003` — which has NO service worker registered, definitively ruling out the SW). The cause was the Sponsor's local extraction/serve directory still containing the OLD build (the new zip extracted elsewhere / served folder not updated), NOT a bad artifact. **Don't keep theorizing about cache — verify the artifact, then take the Sponsor's machine out of the loop:**
1. **Verify the artifact server-side, definitively:** `gh run download <release-run-id> -R <owner>/<repo> -n <artifact-name> -D <tmpdir>` then `grep -alc "<new-sha>" <tmpdir>/index.pck` and `grep -alc "<old-sha>" <tmpdir>/index.pck`. The build SHA is baked as an ASCII string inside `index.pck` (grep `-a` reads it). If the new SHA hits and the old doesn't, the artifact is provably correct — stop blaming the build.
2. **Then serve the verified folder YOURSELF on a fresh port** and hand the Sponsor a click-only URL: `python -m http.server <freshport>` from the extracted-and-verified folder → `http://localhost:<freshport>/?<flags>`. This removes the Sponsor's download/extraction/cache entirely from the loop — the single highest-leverage fix after 3+ wrong-folder soak rounds. (Note: stdlib `http.server` lacks COOP/COEP, fine for the simple interactive `Main.tscn` soak path per § "CLI-agent HTML5 self-soak"; only headless Playwright needs the Node header-fixture server.) Make orch-serve the DEFAULT soak handoff once a Sponsor hits the wrong-folder trap even once.
