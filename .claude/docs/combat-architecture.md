...
**The rule is NOT Area2D-only — it covers `CollisionShape2D`-on-`PhysicsBody2D` adds too.** Splicing *any* node that owns a `CollisionShape2D` into the live tree mid-flush registers that shape with the physics server, and the server's per-shape state pushes — `body_set_shape_disabled` (`godot_physics_server_2d.cpp:654`) and `body_set_shape_as_one_way_collision` (`cpp:663`) — panic with the *same* "Can't change this state while flushing queries" message even though no `Area2D` and no `monitoring` flag is involved. A `CharacterBody2D` mob with a baked-in `CollisionShape2D` child entering the tree during a flush hits this. When the panic fires, the C++ early-returns and the shape is left **unregistered with the server**: the node renders + `_physics_process`-ticks normally, but it has *no physics-collision presence* — `Area2D.get_overlapping_bodies()` / `body_entered` never see it. This was the **Room 05 3-concurrent-chaser freeze (ticket 86c9u1cx1, PR for branch `drew/room05-chaser-freeze`)**: `MultiMobRoom._build()` ran `add_child(_assembly.root)` synchronously from `_ready`, and `_ready` runs inside the *prior* room's `RoomGate.body_entered` flush window (`gate_traversed → room_cleared → Main._on_room_cleared → _load_room_at_index → _world.add_child(room) → MultiMobRoom._ready`). The mob `CollisionShape2D` inserts panicked, the 3 mobs ended up un-hittable (player swings produced zero `Hitbox.hit` against them), no mob died, and the RoomGate never unlocked — Room 05 was unbeatable. PR #183 had deferred the *RoomGate* Area2D fixture pass but explicitly left `_build()` synchronous on the **wrong** rationale that "CharacterBody2D mobs = no Area2D monitoring mutation, safe." Fix: `MultiMobRoom._build()` now only *constructs* the `LevelAssembler.AssemblyResult` (geometry + mobs parented under the **detached** `_assembly.root` — zero physics-server calls, `_assembly.mobs` still fully populated for `Main._wire_room_signals`); the `add_child(_assembly.root)` tree-insertion is split into `_attach_assembly()`, called first in the already-deferred `_assemble_room_fixtures()` pass.
...
2. **Caller-side defer** — `parent.call_deferred("add_child", child)` at the spawn site, or split construction (synchronous, detached) from tree-insertion (deferred). Use when you can't subclass the node, or for `CharacterBody2D`/`StaticBody2D`+`CollisionShape2D` subtrees where there is no monitoring flag to encapsulate (the Room 05 / `MultiMobRoom._attach_assembly` pattern).

**Pattern check for room-lifecycle code:** any room-load path rooted in a `body_entered` chain (`Main._load_room_at_index` is the canonical one — it runs from the prior room's gate traversal) must NOT `add_child` *anything* with a CollisionShape2D — Area2D fixtures AND body subtrees alike — synchronously from `_ready`. Defer the tree-insertion. `Stratum1Room01._build()` has the same synchronous-`add_child` shape and is a latent sibling: Room 01 normally loads at boot (not during a flush) so it is safe today, but the player-death reload path (`Player._die` from a Hitbox `body_entered` → `player_died` → `Main` reload) would route Room 01's `_build()` through a flush — track if death-reload ever surfaces a Room 01 mob-freeze.
