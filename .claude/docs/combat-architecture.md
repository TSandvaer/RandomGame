...
## Sprite-node topology — current ColorRect placeholders, M3 introduces AnimatedSprite2D (load-bearing for art-pass scene-wiring)

**Current state (M1/M2):** every visible combatant — Player and every mob — uses a child node literally named `Sprite` that is a **`ColorRect`**, not an `AnimatedSprite2D`. The node name is aspirational; the type is placeholder. M3's art-pass is therefore **net-new infrastructure introduction** (add AnimatedSprite2D, wire SpriteFrames, hook state→animation), NOT a sprite-swap on existing animation rigging. Tickets that say "swap in sprite" are scope-underestimated — there is nothing to swap into.

**Three seams are already wired and load-bearing for the M3 rewire** — do not remove or rename them when introducing AnimatedSprite2D:

1. **Hit-flash two-branch resolver — `_hit_flash_uses_sprite` toggle.** [`scripts/mobs/Grunt.gd`](../../scripts/mobs/Grunt.gd) / `Charger.gd` / `Shooter.gd` / `Stratum1Boss.gd` each set `_hit_flash_uses_sprite = true` when the child `Sprite` ColorRect resolves on first hit; falls back to `self.modulate` when no Sprite child exists (bare-instanced test mobs). **The branch is load-bearing** — both paths are pinned by `tests/test_combat_visuals.gd` (color-delta assert, NOT `tween_valid`; see Tier 1 corollary above). On the M3 swap, the resolver MUST force the modulate path: an `AnimatedSprite2D` has `modulate` but NO `color` property, so the existing `target.color = rest` write silently null-derefs after an `as ColorRect` cast. Either (a) keep the branch and route AnimatedSprite2D through `.modulate` like the no-Sprite-child fallback, or (b) replace `as ColorRect` with a CanvasItem-typed handle and tween `.modulate` uniformly. Tests pin both branches — update them in the same PR.

2. **Player aim-rotation — `_update_sprite_rotation`.** [`scripts/player/Player.gd`](../../scripts/player/Player.gd) rotates the Sprite child to `_facing.angle()` every `_physics_process` tick (see "Mouse-direction facing" above). A 16×16 ColorRect square is visually rotation-invariant today, so the wiring is mechanical-only — observable via the `[combat-trace] Player.try_attack | FIRED facing=(x,y)` line. On the AnimatedSprite2D swap there are two viable shapes: (a) **continuous rotation** — keep `Sprite.rotation = _facing.angle()` (with the comment's `-PI/2` offset if the art's "default tip-up" convention requires it); fine for symmetric weapons. (b) **8-direction frame selection** — drop `rotation`, instead map `_facing.angle()` to an octant index and `play("idle_N" / "idle_NE" / …)`. Pick (a) for M3 unless the art demands per-direction frames; the `_update_sprite_rotation` call site is the single edit point either way.

   **Resolution (PR #274, 2026-05-18):** M3W-2 picked shape (b) — 8-direction frame selection — and the Sponsor-soak bounce-back surfaced that decoupling `_facing` from non-aim-coupled state animations requires fixing **two parallel surfaces**, not one:

   1. **Animation-name selection** — which `walk_<dir>` / `attack_light_<dir>` anim plays. `_resolve_anim_dir(prefix)` switches on prefix: WALK/IDLE use the velocity octant (or held `_last_anim_dir` on rest), ATTACK/DODGE/HIT/DIE use the `_facing` octant. Fixed in PR #274 v1.
   2. **Sprite-node rotation** — the `AnimatedSprite2D`'s `.rotation` property. Pre-M3 a symmetric ColorRect made `_facing`-tied rotation visually invisible. Post-M3W-2, asymmetric directional frames make node rotation visually apparent — the rotated frame reads as "the character is looking at the cursor" on top of the per-direction art. **Rule: directional frames carry orientation; the sprite-node `rotation` property must stay 0 across ALL states.** `_update_sprite_rotation()` pins `(sprite as CanvasItem).rotation = 0.0` and is the documentation-bearing single edit point against future regressions. Fixed in PR #274 v2.

   Missing the second surface is what made the PR #274 first-attempt soak bounce: the `[combat-trace] Player._play_anim | PLAY anim=walk_n` lines were correct, but the visual still read as "mouse-tracked" because the `AnimatedSprite2D` node itself was rotating in lock-step with `_facing` via the pre-swap aim-rotation hook. Both surfaces must be decoupled together; pin both with paired tests (`tests/test_player_mouse_facing.gd::test_sprite_rotation_stays_zero_across_facing` for the rotation pin; `tests/test_player_animation_wire.gd::test_walk_anim_follows_velocity_not_facing` for the name-selection pin).

   The swing-wedge ColorRect (`_spawn_swing_wedge`, line ~1307) rotates independently via `wedge.rotation = dir.angle()` and is correctly scoped — it's a separate node whose rotation IS load-bearing for the swing-arc cue. Don't conflate "Sprite node rotation = 0" with "all `_facing`-derived rotation is wrong" — the rule applies only to the `AnimatedSprite2D` Sprite child, not to action-coupled cue overlays.

3. **State→animation hook — `Player.gd:34` `signal state_changed(from, to)`.** Already emitted on every state transition (`STATE_GROUNDED` ↔ `STATE_ATTACK` ↔ `STATE_DODGE`, etc.). Pre-built animation seam: a future `_on_state_changed(from, to)` slot can `match to:` and call `Sprite.play("idle" / "attack" / "dodge")`. **Mobs have no equivalent signal yet** — `Shooter._set_state` emits the trace line but no Godot signal. Recommend adding `signal state_changed(from, to)` to mob base/Shooter as a same-PR companion when the AnimatedSprite2D swap lands, for parity with Player.

**M3 pre-swap checklist** (per mob class and Player):
1. Replace the `Sprite` ColorRect node with `AnimatedSprite2D` of the same name (keep the node name — every `_hit_flash_target`, `_update_sprite_rotation`, etc. resolves by `get_node("Sprite")`).
2. Assign the `SpriteFrames` resource (from `assets/sprites/<entity>/_pixellab_anims/`).
3. Update `_play_hit_flash` resolver: force the modulate branch (AnimatedSprite2D has no `color`) OR retype the handle to CanvasItem and tween `.modulate`. Update `_hit_flash_uses_sprite`-branch tests accordingly.
4. Wire `_on_state_changed(from, to)` slot on Player (and equivalent on each mob once mob `state_changed` lands) — map states to animation names.
...
## `char_scale` coupling — two spawn surfaces + reach must scale (load-bearing, PR #426)

`char_scale` (the `?char_scale` debug param / `CHAR_SCALE_PRODUCTION_DEFAULT` in `DebugFlags.gd`, default 0.6, exempting bosses via `_char_scale_is_boss`) scales the player + every non-boss mob's root transform. Two coupling bugs surfaced together when the default moved 0.6→0.48 (PR #426); both generalize to any future scale change or new mob-spawn surface.

**1. ALL mob-spawn surfaces must be scaled — there are TWO.** `Main._apply_char_scale` early-returned when `_current_room == null` — which is exactly the `?s1_assembler=1` / assembler-floor (procgen) state. Result: the player scaled to 0.48 but the assembler-path mobs stayed at 1.0 → a giant grunt towering over a small player. Static-room mobs arrive via `get_spawned_mobs()`; assembler/procgen-path mobs live in `_s1_mobs` / `_s2_mobs`. **Both lists must be run through the scaler (`_scale_mob_list`).** Any new spawn surface (new level type, new floor assembler consumer) must be added to the scale pass, or its mobs render unscaled. Symptom: player correct size, mobs full-size (or vice-versa) on one level type only.

**2. Any attack/reach CONSTANT coupled to a scaled body must itself scale.** `Grunt.ATTACK_RANGE = 28` was scale-independent while the attack hitbox scaled with `char_scale` (~24px contact at 0.48). The mob parked at `dist_to_player ≈ 27` (inside its 28px think-range) and swung into a ~4px **dead-gap** — the scaled hitbox never reached → "0 hits absorbed", combat silently broken. Fix: `_effective_attack_range() = ATTACK_RANGE * scale.x` (applied to Grunt + Charger). **Rule:** this is the melee sibling of the Shooter band-vs-reach invariant above — any range/reach/engagement constant that governs when a *scaled* body makes contact must be multiplied by the body's scale, or a scale change opens a dead-gap. Pin it with a scale-aware GUT test (assert contact at the non-default scale, not just 1.0).

**Test-harness coupling (Playwright):** fixed-duration "walk the player N seconds toward the dummy" helpers are implicitly tuned to one scale's reach; a scale change makes them stop short. Use live-position convergence (`walkPlayerToDummy` reads `latestPlayerPos` and re-issues until within contact) rather than a hardcoded walk duration, so the harness is scale-robust. (PR #426: 5 reach/combat specs failed purely on this until the helper was made convergent.)
...
## Mobs have NO NavigationAgent — solid-prop levels need a grunt-radius BFS gate (load-bearing for level composition, PR #417)

Grunt (and the melee mobs) chase the player by **direct steering** (`velocity = (player_pos - pos).normalized() * speed` → `move_and_slide()`), **not** a `NavigationAgent2D` / nav mesh. There is no pathfinding around obstacles. Consequence for any level that places **solid collision props in the play space** (e.g. the S1 colonnade — pillars/braziers/large rubble as `StaticBody2D`): a solid box that protrudes into a traversal lane can **wedge a chasing mob** against it — the mob steers straight at the player, hits the box, and `move_and_slide` produces little net progress (same zero-net-motion failure mode as the Shooter cornered-kiting bug at §"Shooter state machine"). "The aisle looks clear to the eye" is NOT sufficient.

**Level-composition test gate:** validate any solid-prop arrangement with a **radius-EXPANDED BFS** (inflate obstacles by the moving body's collision radius before the reachability flood), asserting (a) the room is clearable entry→exit, (b) **every mob-spawn tile can reach the player** around the props, AND (c) **the PLAYER's live traversal paths are clear — spawn→entry-gate-trigger and spawn→exit-gate** (PR #417 respin: a center-aisle BFS passed while a SW-corner rubble box wedged the *player's* westward gate-approach walk at Y≈200, so `body_entered` never fired and the room-gate Playwright spec regressed). The wedge bites the player too, not just mobs — model BOTH the mob→player paths and the player→gate paths. A center-point BFS (zero radius) passes arrangements that still wedge a real radius-12 body — the expansion is the load-bearing part. Keep solid-prop footprints at the prop *base*, smaller than the sprite, with ≥2-tile (64 px) gaps between paired obstacles so a mob (radius ~12 px at `char_scale=0.6`) can thread between them. Worked example + test: PR #417 S1 Room02 colonnade.
