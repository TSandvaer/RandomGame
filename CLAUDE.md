...
- **Orchestrator never codes.** Don't read source, grep, trace bugs, or edit code. Dispatch agents from symptoms instead. **R&D lane exception:** MCP-bound generation (pixellab/pixel-mcp) and Sponsor-interactive style/feel iteration are legitimate orchestrator-direct; MCP-free R&D (procedural math, headless engine wiring, scene authoring) dispatches to Devon/Drew. Every R&D burst closes with a harvest PR + peer review + productionization tickets. See `.claude/docs/orchestration-overview.md` § "R&D lane".
