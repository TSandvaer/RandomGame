...
func test_autoload_registered_at_root() -> void:
	...
	assert_true(
		_director.has_method("request"), "request(reason, scale, duration, priority) API present"
	)
...
func test_boot_state_is_clean_scale_1_no_requests() -> void:
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "empty stack → scale 1.0")
	assert_almost_eq(Engine.time_scale, 1.0, 0.001, "Engine.time_scale mirrors director")
	assert_eq(_director.active_reasons().size(), 0, "no active reasons on boot")
...
func test_single_request_applies_scale_then_release_restores() -> void:
	...
	assert_almost_eq(
		_director.current_scale(), 0.5, 0.001, "single request at 0.5 sets effective scale to 0.5"
	)
	assert_almost_eq(Engine.time_scale, 0.5, 0.001, "Engine.time_scale mirrors")
	assert_true(_director.is_active("test_slow"), "is_active reports true")

	_director.release("test_slow")
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "release with empty stack → 1.0")
	assert_almost_eq(Engine.time_scale, 1.0, 0.001, "Engine.time_scale restored")
...
func test_release_unknown_reason_is_silent_noop() -> void:
	...
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "silent no-op on unknown release")
...
func test_release_lower_scale_falls_back_to_higher() -> void:
	...
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "releasing all → 1.0")
...
func test_release_higher_scale_doesnt_change_effective() -> void:
	...
	assert_eq(_scale_changes.size(), 0, "scale_changed should NOT fire (no-op write avoided)")
...
func test_freeze_priority_trumps_all_lower() -> void:
	...
	assert_almost_eq(
		_director.current_scale(), 0.0, 0.001, "freeze (priority 2) trumps narrative (priority 1)"
	)
...
func test_top_priority_bucket_picks_lowest_scale_among_peers() -> void:
	...
	assert_almost_eq(
		_director.current_scale(), 0.3, 0.001, "within top priority bucket, lowest scale (0.3) wins"
	)
...
func test_auto_release_after_duration_expires() -> void:
	...
	assert_false(_director.is_active("short"), "request auto-released after duration expired")
	assert_almost_eq(
		_director.current_scale(), 1.0, 0.001, "Engine.time_scale restored after auto-release"
	)
...
func test_freeze_auto_release_works_despite_scale_0() -> void:
	...
	assert_false(
		_director.is_active("freeze"), "freeze auto-released despite scale=0.0 during the window"
	)
	assert_almost_eq(
		_director.current_scale(),
		1.0,
...
func test_rerequest_same_reason_replaces_scale_and_priority() -> void:
	...
	assert_almost_eq(
		_director.current_scale(), 0.3, 0.001, "re-request replaces prior; new scale takes effect"
	)
	assert_eq(
		_director.active_reasons().size(), 1, "single active entry — no stacking on re-request"
	)
...
func test_rerequest_cancels_prior_auto_release_timer() -> void:
	...
	assert_true(_director.is_active("renew"), "re-requested entry survives the old timer's fire")
	assert_almost_eq(
		_director.current_scale(), 0.4, 0.001, "effective scale reflects the NEW request"
	)
...
func test_reset_clears_all_requests_and_restores_1_0() -> void:
	...
	_director.reset()
	assert_eq(_director.active_reasons().size(), 0, "reset clears the stack")
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "reset restores 1.0")
...
func test_freeze_sets_0_with_priority_2_default() -> void:
	...
	assert_almost_eq(_director.current_scale(), 0.0, 0.001, "freeze() sets scale to 0.0")
	assert_true(_director.is_active("freeze"), "default reason is 'freeze'")
...
func test_freeze_custom_reason_allows_coexistence() -> void:
	...
	assert_eq(_director.active_reasons().size(), 2, "two distinct freeze reasons can coexist")
	# Releasing one leaves the other active.
	_director.release("boss_died_freeze")
	assert_almost_eq(
		_director.current_scale(), 0.0, 0.001, "remaining freeze still pins scale at 0.0"
	)
...
func test_empty_reason_request_emits_warning_and_does_nothing() -> void:
	...
	assert_eq(_director.active_reasons().size(), 0, "empty reason rejected — no entry added")
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "scale unchanged")
...
func test_scale_above_max_clamps_and_warns() -> void:
	...
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "scale > 1.0 clamped to 1.0")
...
func test_scale_changed_fires_only_on_actual_change() -> void:
	...
	assert_eq(_scale_changes.size(), 0, "no-op scale write does not fire scale_changed")
...
func test_t2_t3_conflict_phase_transition_wins_over_hit_pause() -> void:
	...
	_director.request("phase_transition_2", 0.3, 0.6, _director.PRIORITY_NARRATIVE)
	# Mid-phase-transition, a hit lands and asks for a hit-pause.
	_director.request("hit_pause", 0.05, 0.06, _director.PRIORITY_DEFAULT)
...
func test_rerequest_with_identical_scale_priority_does_not_self_erase() -> void:
	...
	assert_true(
		_director.is_active("renew_same"),
...
func test_freeze_self_extension_via_rerequest_does_not_self_erase() -> void:
	...
	await get_tree().create_timer(0.12, true, false, true).timeout
	assert_true(
		_director.is_active("extending_freeze"),
		"extended freeze must survive the original timer's stale fire"
	)
	assert_almost_eq(_director.current_scale(), 0.0, 0.001, "freeze still active after extension")
...
func test_nan_scale_is_rejected_with_warning() -> void:
	...
	assert_eq(_director.active_reasons().size(), 0, "NaN scale rejected — no entry added")
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "Engine.time_scale untouched")
	# Belt: the engine itself must not have been polluted with NaN.
	assert_false(is_nan(Engine.time_scale), "Engine.time_scale is NOT NaN")


func test_positive_inf_scale_is_rejected_with_warning() -> void:
	_warn_guard.expect_warning("non-finite scale")
	_director.request("inf_probe", INF, 0.0)
	assert_eq(_director.active_reasons().size(), 0, "+Inf scale rejected — no entry added")
	assert_almost_eq(_director.current_scale(), 1.0, 0.001, "Engine.time_scale untouched")
