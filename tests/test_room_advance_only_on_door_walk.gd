...
func test_killing_all_mobs_without_gate_trigger_does_not_unlock() -> void:
	...
	assert_signal_not_emitted(
		g, "gate_unlocked", "room_cleared must NOT fire if gate was never locked"
	)
...
func test_gate_locks_on_player_body_entry_with_mobs_alive() -> void:
	...
	assert_signal_not_emitted(
		g, "gate_unlocked", "gate must stay LOCKED while the mob is alive — no premature advance"
	)
...
func test_lock_then_kill_multiple_mobs_unlocks_on_last() -> void:
	...
	await get_tree().process_frame
	assert_signal_not_emitted(
		g, "gate_unlocked", "gate must stay locked after first mob — second still alive"
	)
	m2.die()
	await get_tree().process_frame
...
func test_gate_collision_mask_targets_player_layer_only() -> void:
	...
	add_child_autofree(g)
	assert_true(
		(g.collision_mask & RoomGate.LAYER_PLAYER) != 0, "gate must mask the player physics layer"
	)
	assert_eq(g.collision_layer, 0, "gate emits no collisions itself — zero collision_layer")
	# Hitbox sits on LAYER_PLAYER_HITBOX (bit 3 = 4). Gate must NOT mask it.
	const LAYER_PLAYER_HITBOX: int = 1 << 2
	assert_eq(
		g.collision_mask & LAYER_PLAYER_HITBOX,
		0,
