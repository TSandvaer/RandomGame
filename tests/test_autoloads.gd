...
func test_save_schema_version_constant_is_positive_int() -> void:
	...
	assert_true(const_map.has("SCHEMA_VERSION"), "Save script must declare SCHEMA_VERSION constant")
...
func test_dialogue_controller_autoload_registered() -> void:
	...
	assert_not_null(
		dc, "DialogueController must be registered as autoload (project.godot [autoload])"
	)
	for method: String in [
		"open",
		"advance_line",
...
func test_gamestate_autoload_when_present() -> void:
	...
	if gs == null:
		...
		pending(
			"GameState autoload not yet registered — see automated-smoke-plan.md tu-autoload-01"
		)
		return
