...
func test_all_five_buses_exist_by_name() -> void:
	...
	assert_ne(
		AudioServer.get_bus_index(BUS_MASTER), -1, "Master bus must exist (default_bus_layout.tres)"
	)
	assert_ne(
		AudioServer.get_bus_index(BUS_BGM), -1, "BGM bus must exist (per audio-direction.md §3)"
	)
	assert_ne(
	...
	assert_ne(
		AudioServer.get_bus_index(BUS_SFX), -1, "SFX bus must exist (per audio-direction.md §3)"
	)
	assert_ne(
		AudioServer.get_bus_index(BUS_UI), -1, "UI bus must exist (per audio-direction.md §3)"
	)
...
func test_master_bus_is_index_zero() -> void:
	...
	assert_eq(
		AudioServer.get_bus_index(BUS_MASTER), 0, "Master bus must be index 0 (Godot convention)"
	)
...
func test_master_bus_at_zero_db() -> void:
	...
	assert_almost_eq(db, 0.0, DB_EPSILON, "Master bus must be 0 dB (final output reference)")


func test_bgm_bus_at_minus_twelve_db() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_BGM)
	var db: float = AudioServer.get_bus_volume_db(idx)
	assert_almost_eq(db, -12.0, DB_EPSILON, "BGM bus must be -12 dB (audio-direction.md §3)")


func test_ambient_bus_at_minus_eighteen_db() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_AMBIENT)
	var db: float = AudioServer.get_bus_volume_db(idx)
	assert_almost_eq(db, -18.0, DB_EPSILON, "Ambient bus must be -18 dB (audio-direction.md §3)")


func test_sfx_bus_at_minus_six_db() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_SFX)
	var db: float = AudioServer.get_bus_volume_db(idx)
	assert_almost_eq(db, -6.0, DB_EPSILON, "SFX bus must be -6 dB (audio-direction.md §3)")


func test_ui_bus_at_minus_ten_db() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_UI)
	var db: float = AudioServer.get_bus_volume_db(idx)
	assert_almost_eq(db, -10.0, DB_EPSILON, "UI bus must be -10 dB (audio-direction.md §3)")
...
func test_bgm_sends_to_master() -> void:
	...
	assert_eq(send, BUS_MASTER, "BGM bus must send to Master (audio-direction.md §3)")


func test_ambient_sends_to_master() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_AMBIENT)
	var send: String = AudioServer.get_bus_send(idx)
	assert_eq(send, BUS_MASTER, "Ambient bus must send to Master (audio-direction.md §3)")


func test_sfx_sends_to_master() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_SFX)
	var send: String = AudioServer.get_bus_send(idx)
	assert_eq(send, BUS_MASTER, "SFX bus must send to Master (audio-direction.md §3)")


func test_ui_sends_to_master() -> void:
	var idx: int = AudioServer.get_bus_index(BUS_UI)
	var send: String = AudioServer.get_bus_send(idx)
	assert_eq(send, BUS_MASTER, "UI bus must send to Master (audio-direction.md §3)")
