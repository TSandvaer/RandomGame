...
func test_equip_same_instance_twice_idempotent() -> void:
	...
	p.equip_item(instance)
	assert_eq(
		_ps().get_stat(&"edge"), 5, "re-equipping the same instance is a no-op (no double-apply)"
	)
...
func test_swift_affix_increases_walk_speed() -> void:
	...
	p.equip_item(instance)
	assert_almost_eq(
		p.get_walk_speed(), Player.WALK_SPEED + 4.0, 0.001, "swift +4 -> walk_speed += 4"
	)
	p.unequip_item(Player.SLOT_WEAPON)
	assert_almost_eq(
		p.get_walk_speed(), Player.WALK_SPEED, 0.001, "unequip reverses move_speed bonus"
	)
...
func test_authored_swift_affix_uses_move_speed() -> void:
	...
	assert_eq(swift.stat_modified, &"move_speed", "swift affix targets move_speed (M1 task spec)")
...
func test_authored_vital_affix_uses_vigor() -> void:
	...
	assert_eq(vital.stat_modified, &"vigor", "vital affix targets vigor (M1 task spec)")
...
func test_authored_keen_affix_uses_edge() -> void:
	...
	assert_eq(keen.stat_modified, &"edge", "keen affix targets edge (M1 task spec)")
