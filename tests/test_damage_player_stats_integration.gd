...
func test_player_get_edge_reads_from_player_stats_autoload() -> void:
	...
	assert_eq(p.get_edge(), 0, "Player.get_edge() returns 0 when PlayerStats.edge=0")
	# Allocate edge in PlayerStats; Player.get_edge() reflects it.
	_ps().add_stat(&"edge", 5)
	assert_eq(p.get_edge(), 5, "Player.get_edge() reads from PlayerStats autoload")
...
func test_player_get_vigor_reads_from_player_stats_autoload() -> void:
	...
	assert_eq(p.get_vigor(), 8, "Player.get_vigor() reads from PlayerStats autoload")
...
func test_player_attack_damage_scales_with_player_stats_edge() -> void:
	...
	assert_eq(dmg_five, 12, "edge=5 boosts light damage to floor(10*1.25)=12")
...
func test_player_try_attack_uses_current_player_stats_edge() -> void:
	...
	assert_eq(integrated_dmg, 15, "Player.get_edge() + Damage formula yields 15 at edge=10")
...
func test_mob_damage_scales_with_player_stats_vigor() -> void:
	...
	assert_eq(
		DamageScript.compute_mob_damage(m, p.get_vigor()), 10, "vigor=0 -> mob deals full base 10"
	)
	# vigor=10: 10 * (1 - 0.20) = 8.
	_ps().add_stat(&"vigor", 10)
	assert_eq(
		DamageScript.compute_mob_damage(m, p.get_vigor()),
		8,
	...
	assert_eq(
		DamageScript.compute_mob_damage(m, p.get_vigor()), 5, "vigor=25 -> 50% cap, mob deals 5"
	)
...
func test_no_player_stats_falls_back_to_local_field() -> void:
	...
	assert_eq(
		p.get_edge(), 0, "PlayerStats autoload value wins over Player.set_stat-modified _edge"
	)
...
func test_negative_player_stats_input_doesnt_underflow() -> void:
	...
	var w: ItemDef = _make_weapon(10)
	assert_eq(
		DamageScript.compute_player_damage(w, _ps().get_stat(&"edge"), Player.ATTACK_LIGHT), 12
	)
