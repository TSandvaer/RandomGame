...
func _make_weapon_instance(
	id: StringName = &"test_sword", damage_override: int = 5
) -> ItemInstance:
	var def: ItemDef = (
		ContentFactory
		. make_item_def(
...
func test_pickup_weapon_with_empty_slot_auto_equips() -> void:
	...
	assert_not_null(equipped, "picking up a weapon with an empty weapon slot must auto-equip it")
	assert_eq(
		equipped.def.id, &"iron_sword", "the auto-equipped weapon is the picked-up iron_sword"
	)
	# The sword moved from grid → slot: it is NOT also sitting in the grid.
	assert_false(
		_inv().get_items().has(sword),
...
func test_pickup_weapon_with_slot_filled_does_not_auto_swap() -> void:
	...
	assert_eq(
		_inv().get_equipped(&"weapon") as ItemInstance,
		first_sword,
	...
	assert_true(
		_inv().get_items().has(second_sword),
		"the second picked-up weapon lands in the grid, not the slot"
	)
	assert_eq(_inv().get_items().size(), 1, "grid holds exactly the un-equipped second weapon")
...
func test_pickup_armor_never_auto_equips() -> void:
	...
	assert_null(_inv().get_equipped(&"weapon"), "picking up armor must NOT touch the weapon slot")
	assert_null(
		_inv().get_equipped(&"armor"),
		(
	...
	assert_true(
		_inv().get_items().has(armor), "armor lands in the grid like any other non-weapon pickup"
	)
...
func test_iron_sword_tres_loads_and_resolves() -> void:
	...
	assert_not_null(resolved, "ContentRegistry must resolve iron_sword — null means BB-2 repeat")
...
func test_retired_bandaid_methods_are_gone() -> void:
	...
	assert_false(
		_inv().has_method("equip_starter_weapon_if_needed"),
		(
	...
	assert_false(
		_inv().has_method("_seed_starting_inventory"),
		(
