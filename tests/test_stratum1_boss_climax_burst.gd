...
func test_t16_a_burst_one_shot_but_not_explosive() -> void:
	...
	assert_true(burst.one_shot, "T16-A: burst is one_shot (single emission window, not looping)")
	assert_lt(
		burst.explosiveness,
		1.0,
	...
	assert_almost_eq(
		burst.explosiveness,
		Stratum1Boss.CLIMAX_BURST_EXPLOSIVENESS,
		0.001,
...
func test_t16_b_lifetime_matches_f2_window() -> void:
	...
	assert_almost_eq(
		Stratum1Boss.CLIMAX_BURST_LIFETIME,
		Vignette.F2_BOSS_DEFEAT_DURATION,
		0.001,
...
func test_t16_d_ramp_has_impact_frame_at_offset_zero() -> void:
	...
	assert_almost_eq(
		start.r,
		Stratum1Boss.CLIMAX_BURST_IMPACT_FLASH.r,
		0.005,
		"T16-D: ramp[0] R matches CLIMAX_BURST_IMPACT_FLASH constant"
	)
	assert_almost_eq(
		start.g,
		Stratum1Boss.CLIMAX_BURST_IMPACT_FLASH.g,
		0.005,
		"T16-D: ramp[0] G matches CLIMAX_BURST_IMPACT_FLASH"
	)
	assert_almost_eq(
		start.b,
		Stratum1Boss.CLIMAX_BURST_IMPACT_FLASH.b,
		0.005,
...
func test_t16_f_z_index_above_boss_sprite() -> void:
	...
	assert_eq(
		burst.z_index,
		Stratum1Boss.CLIMAX_BURST_Z_INDEX,
...
func test_t16_g_gravity_rises_no_horizontal_drift() -> void:
	...
	assert_lt(
		burst.gravity.y, 0.0, "T16-G: gravity y < 0 — particles rise (ember-rise design intent)"
	)
	assert_almost_eq(
		burst.gravity.y,
		Stratum1Boss.CLIMAX_BURST_GRAVITY_Y,
		0.001,
...
func test_t16_i_burst_room_parented_not_boss_parented() -> void:
	...
	assert_not_null(room_burst, "T16-I: burst is under the room (deferred add_child landed)")
	assert_null(boss_burst, "T16-I: burst is NOT under the boss (the boss is about to fade out)")
