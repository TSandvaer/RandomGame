...
func test_save_migrated_v0_then_reload_round_trips() -> void:
	...
	# SCHEMA_VERSION (v3 as of 2026-05-02 — added stats block + unspent +
	# first_level_up_seen for the level-up allocation UI). Reload that and
	# the schema_version on disk MUST match SCHEMA_VERSION — we never re-
	# emit an old version from a current runtime.
	_install_fixture_at_slot(FIXTURE_V0, TEST_SLOT)
