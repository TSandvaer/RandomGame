...
func test_s2_room03_mob_mix_one_stoker_one_charger_one_shooter() -> void:
	pending(
		"awaiting W3-T2 — assert get_spawned_mobs() composition: 1 Stoker + 1 Charger + 1 Shooter"
	)
...
func test_s2_room02_west_entry_east_exit_ports() -> void:
	pending(
		"awaiting W3-T2 — assert WEST/EAST port positions per Drew's level-chunks.md M2 checklist"
	)
