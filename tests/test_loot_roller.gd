...
func test_tier_modifier_overflow_clamps_to_t6() -> void:
	...
	assert_eq(
		drops[0].rolled_tier, ItemDef.Tier.T6, "tier modifier overflow clamps to T6, not crashes"
	)
		...
		. make_loot_table(
			{
				"entries":
				[
					ContentFactory.make_loot_entry(
						ContentFactory.make_item_def({"id": &"a"}), 0.5, 0
					),
					ContentFactory.make_loot_entry(
						ContentFactory.make_item_def({"id": &"b"}), 0.5, 0
					),
					ContentFactory.make_loot_entry(
						ContentFactory.make_item_def({"id": &"c"}), 0.5, 0
					),
				],
				"roll_count": -1,
			}
...
func test_weighted_pick_mode_produces_exactly_n() -> void:
	...
	for _i in 20:
		assert_eq(
			r.roll(t).size(), 3, "weighted-pick mode always produces exactly roll_count drops"
		)
...
func test_authored_grunt_drops_table_rolls_cleanly() -> void:
	...
	for _i in 200:
		...
		for it: ItemInstance in drops:
			assert_true(
				allowed.has(it.def.id), "rolled id %s is one of the authored M1 items" % it.def.id
			)
		...
		for it: ItemInstance in r.roll(table):
			match it.rolled_tier:
				...
				ItemDef.Tier.T3:
					t3 += 1
				_:
					assert_true(false, "unexpected tier on grunt drop: %d" % it.rolled_tier)
