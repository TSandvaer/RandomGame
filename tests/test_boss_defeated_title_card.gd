...
func test_root_mouse_filter_is_ignore() -> void:
	...
	var card: BossDefeatedTitleCard = _make_card()
	assert_eq(
		card.get_root_control().mouse_filter,
...
func test_subtitle_text_is_locked_constant() -> void:
	var card: BossDefeatedTitleCard = _make_card()
	assert_eq(
		card.get_subtitle_label().text,
		BossDefeatedTitleCardScript.SUBTITLE_TEXT,
...
func test_title_template_includes_period() -> void:
	...
	assert_eq(
		BossDefeatedTitleCardScript.TITLE_TEMPLATE,
		"The %s falls.",
...
func test_show_for_falls_back_on_null_boss() -> void:
	...
	card.show_for(null)
	assert_eq(
		card.get_title_label().text, "The Warden falls.", "null boss falls back to 'Warden' literal"
	)
...
func test_pre_fade_delay_is_one_two_seconds() -> void:
	...
	assert_almost_eq(
		BossDefeatedTitleCardScript.PRE_FADE_DELAY,
		1.2,
		0.001,
...
func test_fade_durations_are_locked() -> void:
	...
	assert_almost_eq(
		BossDefeatedTitleCardScript.FADE_IN_DURATION, 0.4, 0.001, "fade-in duration locked to 0.4 s"
	)
	assert_almost_eq(
		BossDefeatedTitleCardScript.HOLD_DURATION, 0.8, 0.001, "hold duration locked to 0.8 s"
	)
	assert_almost_eq(
		BossDefeatedTitleCardScript.FADE_OUT_DURATION,
		0.4,
		0.001,
...
func test_card_layer_separate_from_descend_screen() -> void:
	...
	# this ordering.
	var card: BossDefeatedTitleCard = _make_card()
	assert_lt(card.layer, 100, "card CanvasLayer < DescendScreen (100) so descend paints over card")
