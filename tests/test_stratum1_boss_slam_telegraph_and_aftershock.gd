...
func _arm_slam_telegraph() -> Array:
	...
	assert_eq(b.get_state(), Stratum1Boss.STATE_TELEGRAPHING_SLAM, "boss armed slam-telegraph")
	return [b, p]
...
func test_slam_telegraph_spawns_indicator_child() -> void:
	...
	assert_not_null(
		indicator, "T5-1: slam-telegraph spawns a SlamTelegraphIndicator child of the boss"
	)
	assert_true(
		indicator is SlamTelegraphIndicator, "T5-1: child is the SlamTelegraphIndicator class"
	)
	# Parent-relative position (boss-centered) per scope-doc AC.
	assert_almost_eq(
		(indicator as Node2D).position.x,
		0.0,
		0.001,
		"T5-1: indicator is parent-relative at boss origin (x)"
	)
	assert_almost_eq(
		(indicator as Node2D).position.y,
		0.0,
		0.001,
...
func test_slam_telegraph_indicator_frees_on_slam_fire() -> void:
	...
	assert_eq(
		b.get_state(),
		Stratum1Boss.STATE_SLAM_RECOVERY,
	...
	if indicator_after != null:
		assert_true(
			indicator_after.is_queued_for_deletion(),
...
func test_slam_telegraph_indicator_frees_on_boss_die_mid_telegraph() -> void:
	...
	assert_true(
		indicator.is_queued_for_deletion(),
...
func test_slam_telegraph_re_entry_does_not_stack_indicators() -> void:
	...
	for child in b.get_children():
		if child is SlamTelegraphIndicator and not child.is_queued_for_deletion():
			count += 1
	assert_eq(count, 1, "T5-4: at most one live SlamTelegraphIndicator child after re-entry")
...
func test_slam_indicator_color_matches_scope_ac() -> void:
	...
	assert_almost_eq(c.r, 1.0, 0.005, "T5-5: indicator R channel matches #FF (1.0)")
	assert_almost_eq(c.g, 0.4157, 0.005, "T5-5: indicator G channel matches #6A (0.416)")
	assert_almost_eq(c.b, 0.1647, 0.005, "T5-5: indicator B channel matches #2A (0.165)")
	assert_almost_eq(c.a, 0.5, 0.001, "T5-5: indicator alpha 0.5 per scope AC")
	# HTML5 HDR-clamp invariant — every channel must be in [0, 1].
	assert_true(
		c.r <= 1.0 and c.g <= 1.0 and c.b <= 1.0,
		"T5-5: all RGB channels sub-1.0 per html5-export.md HDR-clamp rule"
	)
	assert_true(c.r >= 0.0 and c.g >= 0.0 and c.b >= 0.0, "T5-5: all RGB channels non-negative")
...
func test_slam_fire_spawns_aftershock_burst_parented_to_room() -> void:
	...
	for child in room.get_children():
		if child is CPUParticles2D:
			burst = child
			break
	assert_not_null(burst, "T6-1: slam-fire spawns a CPUParticles2D burst parented to the room")
	# Burst is NOT a child of the boss — that's the contract: persists past
	# boss queue_free.
	for child in b.get_children():
		assert_false(
			child is CPUParticles2D,
...
func test_slam_aftershock_burst_config_matches_scope_ac() -> void:
	...
	assert_almost_eq(
		burst.initial_velocity_min,
		Stratum1Boss.SLAM_AFTERSHOCK_VELOCITY_MIN,
		0.001,
		"T6-2: aftershock min velocity tracks SLAM_AFTERSHOCK_VELOCITY_MIN"
	)
	assert_almost_eq(
		burst.initial_velocity_max,
		Stratum1Boss.SLAM_AFTERSHOCK_VELOCITY_MAX,
		0.001,
	...
	assert_true(burst.one_shot, "T6-2: aftershock is one-shot (does not loop)")
	assert_true(burst.emitting, "T6-2: aftershock starts emitting on spawn")
	...
	assert_almost_eq(
		decay.r,
		Stratum1Boss.EMBER_LIGHT.r,
		0.01,
		"T6-2 v7: ramp@DECAY_OFFSET matches EMBER_LIGHT (warm decay through)"
	)
	# End = EMBER_DEEP.
	assert_almost_eq(
		end.r, Stratum1Boss.EMBER_DEEP.r, 0.005, "T6-2 v7: ramp end matches EMBER_DEEP R"
	)
	...
	assert_almost_eq(
		burst.global_position.x, 100.0, 0.5, "T6-2: aftershock origin x matches boss position"
	)
	assert_almost_eq(
		burst.global_position.y, 200.0, 0.5, "T6-2: aftershock origin y matches boss position"
	)
...
func test_slam_indicator_strobe_constants_are_sane() -> void:
	assert_true(
		Stratum1Boss.SLAM_INDICATOR_STROBE_LOW < Stratum1Boss.SLAM_INDICATOR_STROBE_HIGH,
		"strobe LOW < HIGH so the pulse oscillates"
	)
	assert_true(Stratum1Boss.SLAM_INDICATOR_STROBE_HIGH <= 1.0, "strobe HIGH ≤ 1.0 — Color.a clamp")
	assert_true(Stratum1Boss.SLAM_INDICATOR_STROBE_LOW >= 0.0, "strobe LOW ≥ 0.0 — Color.a clamp")
	assert_true(
		Stratum1Boss.SLAM_INDICATOR_STROBE_HZ >= 3.0, "strobe Hz ≥ 3 — above static-decoration read"
	)
	assert_true(
		Stratum1Boss.SLAM_INDICATOR_STROBE_HZ <= 10.0,
...
func test_boss_hp_mult_clamps_extreme_inputs() -> void:
	...
	assert_eq(b1.hp_max, 30, "sub-MIN input clamps to BOSS_HP_MULT_MIN=0.05 → 600 × 0.05 = 30")
	...
	assert_eq(b2.hp_max, 3000, "super-MAX input clamps to BOSS_HP_MULT_MAX=5.0 → 600 × 5.0 = 3000")
	df.reset_boss_hp_mult_for_test()
...
func test_start_room_clamps_out_of_range() -> void:
	...
	assert_eq(
		int(df.start_room), -1, "negative input resets to START_ROOM_DEFAULT=-1 (no override)"
	)
	df.reset_start_room_for_test()
...
func test_slam_aftershock_burst_self_frees_on_finished() -> void:
	...
	assert_true(
		burst.finished.is_connected(queue_free_callable),
		(
...
func test_slam_impact_flash_runs_on_slam_fire_bare_instance() -> void:
	...
	assert_null(b._slam_impact_flash_tween, "T6-v7: no slam-impact flash tween before slam-fire")
	b._physics_process(Stratum1Boss.SLAM_TELEGRAPH_DURATION + 0.01)
	# Post-slam-fire: flash tween armed + valid.
	assert_not_null(
		b._slam_impact_flash_tween, "T6-v7: slam-impact flash tween created on slam-fire"
	)
	assert_true(
		b._slam_impact_flash_tween.is_valid(),
...
func test_slam_impact_flash_constants_are_html5_safe() -> void:
	...
	assert_lte(
		Stratum1Boss.SLAM_IMPACT_FLASH_TINT.r,
		1.0,
		"T6-v7: SLAM_IMPACT_FLASH_TINT.r <= 1.0 (HTML5 HDR clamp)"
	)
	assert_lte(
		Stratum1Boss.SLAM_IMPACT_FLASH_TINT.g,
		1.0,
		"T6-v7: SLAM_IMPACT_FLASH_TINT.g <= 1.0 (HTML5 HDR clamp)"
	)
	assert_lte(
		Stratum1Boss.SLAM_IMPACT_FLASH_TINT.b,
		1.0,
