...
func test_sprite_frames_has_all_eight_wake_directions() -> void:
	...
	for suffix in DIR_SUFFIXES:
		...
		assert_true(sf.has_animation(key),
...
func test_each_wake_direction_has_five_frames() -> void:
	...
	for suffix in DIR_SUFFIXES:
		var key: StringName = StringName("wake_%s" % suffix)
		assert_eq(sf.get_frame_count(key), EXPECTED_WAKE_FRAMES,
			"wake_%s must have %d frames (PixelLab getting-up template)" % [suffix, EXPECTED_WAKE_FRAMES])
...
func test_wake_animations_are_one_shot() -> void:
	...
	for suffix in DIR_SUFFIXES:
		var key: StringName = StringName("wake_%s" % suffix)
		assert_false(sf.get_animation_loop(key),
			"wake_%s must be one-shot (loop=false)" % suffix)
...
func test_wake_animation_speed_lands_inside_uma_bi06_band() -> void:
	...
	for suffix in DIR_SUFFIXES:
		var key: StringName = StringName("wake_%s" % suffix)
		assert_eq(sf.get_animation_speed(key), EXPECTED_WAKE_FPS,
...
func test_wake_duration_constant_matches_frame_math() -> void:
	...
	assert_almost_eq(Stratum1Boss.WAKE_DURATION, derived, 0.001,
		"WAKE_DURATION constant must equal frames/fps")
	assert_almost_eq(Stratum1Boss.WAKE_DURATION, EXPECTED_WAKE_DURATION_S, 0.001,
...
func test_wake_call_transitions_dormant_to_waking() -> void:
	...
	assert_signal_emit_count(b, "boss_woke", 1,
		"boss_woke fires on DORMANT -> WAKING transition (Beat 3)")
...
func test_set_state_plays_wake_anim_on_animated_sprite() -> void:
	...
	assert_eq(sprite.animation, StringName("wake_s"),
		"AnimatedSprite2D plays wake_s (default dir without player)")
	assert_true(sprite.is_playing(),
		"AnimatedSprite2D is_playing() must be true after wake()")
...
func test_wake_animation_uses_facing_dir_when_player_present() -> void:
	...
	b.set_player(p)
	b.wake()
	assert_eq(sprite.animation, StringName("wake_e"),
