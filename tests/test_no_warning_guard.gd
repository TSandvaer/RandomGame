...
func test_warning_bus_autoload_is_registered() -> void:
	...
	assert_true(bus.has_signal("warning_emitted"), "WarningBus must expose warning_emitted signal")
	assert_true(bus.has_signal("error_emitted"), "WarningBus must expose error_emitted signal")
...
func test_guard_catches_deliberate_warning_via_warning_bus() -> void:
	...
	guard.attach()
	_bus().warn("TEST-86c9uf0mm: deliberate canary warning — guard must catch this", "test-canary")
	assert_eq(
		guard.captured_count(),
		1,
		"NoWarningGuard must capture a WarningBus.warn() call (negative-assertion sanity)"
	)
	var texts: Array = guard.get_captured_texts()
	assert_true(
		(texts[0] as String).find("deliberate canary warning") >= 0,
...
func test_expect_warning_lets_matching_warning_pass_assert_clean() -> void:
	...
	_bus().warn(
		"WARNING: this is the known opt-out pattern in the middle of the message", "test-opt-out"
	)
...
func test_expect_warning_mismatch_does_not_consume_real_warning() -> void:
	...
	_bus().warn(
		"genuine warning text that does not contain the registered pattern", "test-mismatch"
	)
...
func test_guard_without_attach_captures_nothing() -> void:
	...
	assert_eq(guard.captured_count(), 0, "unattached guard captures nothing (no silent passes)")
