...
func test_room_entry_fires_wasd_beat() -> void:
	...
	assert_true(
		room.get_tutorial_beat_emitted(&"wasd"), "WASD latch flipped on room-entry deferred wire"
	)
	assert_signal_emit_count(
		bus, "tutorial_beat_requested", 1, "bus saw exactly one beat-request emit (the WASD beat)"
	)
...
func test_wasd_beat_is_one_shot() -> void:
	...
	assert_false(fired_again, "second WASD emit attempt is a no-op (latch flipped on first fire)")
...
func test_movement_detected_fires_dodge_beat() -> void:
	...
	assert_false(room.get_tutorial_beat_emitted(&"dodge"), "dodge beat NOT fired pre-movement")
	...
	assert_true(
		room.get_tutorial_beat_emitted(&"dodge"),
...
func test_dodge_fires_lmb_strike_beat() -> void:
	...
	assert_false(
		room.get_tutorial_beat_emitted(&"lmb_strike"), "lmb_strike beat NOT fired pre-dodge"
	)
	...
	assert_true(
		room.get_tutorial_beat_emitted(&"lmb_strike"),
...
func test_dummy_death_fires_rmb_heavy_beat() -> void:
	...
	assert_false(
		room.get_tutorial_beat_emitted(&"rmb_heavy"), "rmb_heavy beat NOT fired pre-dummy-death"
	)
	...
	assert_true(
		room.get_tutorial_beat_emitted(&"rmb_heavy"),
...
func test_overlay_defaults_match_uma_beat_4_spec() -> void:
	...
	assert_almost_eq(
		plate.color.a, 0.0, 0.001, "plate alpha = 0 (Uma Beat 4 'no panel background')"
	)
	...
	assert_almost_eq(
		TutorialPromptOverlay.COLOR_TEXT.a,
		0.6,
		0.001,
