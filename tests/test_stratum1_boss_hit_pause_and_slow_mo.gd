...
func test_light_hit_fires_hit_pause_freeze_60ms() -> void:
	...
	assert_true(
		_director.is_active(Stratum1Boss.TSD_REASON_HIT_PAUSE),
		"hit-pause reason is active after non-fatal light hit"
	)
	assert_almost_eq(_director.current_scale(), 0.0, 0.001, "freeze drove Engine.time_scale to 0.0")
...
func test_heavy_hit_fires_hit_pause_freeze_100ms() -> void:
	...
	b.take_damage(10, Vector2.ZERO, p)
	assert_true(
		_director.is_active(Stratum1Boss.TSD_REASON_HIT_PAUSE), "hit-pause active after heavy hit"
	)
...
func test_phase_transition_state_hit_does_not_fire_hit_pause() -> void:
	...
	assert_eq(b.get_state(), Stratum1Boss.STATE_PHASE_TRANSITION, "boss enters phase-transition")
...
func test_already_dead_hit_does_not_fire_hit_pause() -> void:
	...
	assert_false(
		_director.is_active(Stratum1Boss.TSD_REASON_FINAL_FREEZE),
...
func test_lethal_hit_fires_final_freeze_300ms() -> void:
	...
	assert_true(
		_director.is_active(Stratum1Boss.TSD_REASON_FINAL_FREEZE), "lethal hit fires final-freeze"
	)
	assert_almost_eq(_director.current_scale(), 0.0, 0.001, "final-freeze drives scale to 0.0")
...
func test_final_freeze_request_appears_in_request_changes() -> void:
	...
	var final_freeze_events: Array = _request_changes.filter(
		func(e): return e["reason"] == Stratum1Boss.TSD_REASON_FINAL_FREEZE
	)
	assert_gt(
		final_freeze_events.size(), 0, "lethal hit emits request_changed for FINAL_FREEZE reason"
	)
	assert_eq(
		final_freeze_events[0]["op"],
		"added",
...
func test_phase_2_transition_fires_slow_mo_request() -> void:
	...
	assert_true(
		_director.is_active(Stratum1Boss.TSD_REASON_PHASE_TRANSITION),
...
func test_phase_transition_slow_mo_fires_once_under_hit_spam() -> void:
	...
	var phase_events: Array = _request_changes.filter(
		func(e): return e["reason"] == Stratum1Boss.TSD_REASON_PHASE_TRANSITION
	)
	...
	assert_eq(
		phase_events.size(), 1, "phase-transition slow-mo request emits exactly once under hit-spam"
	)
...
func test_hit_pause_priority_trumps_phase_transition_priority() -> void:
	...
	assert_almost_eq(_director.current_scale(), 0.3, 0.001, "phase-transition alone → scale 0.3")
...
func test_t2_constants_match_priya_ac() -> void:
	...
	assert_almost_eq(
		Stratum1Boss.HIT_PAUSE_LIGHT_DURATION,
		0.060,
		0.0001,
		"HIT_PAUSE_LIGHT_DURATION = 60 ms (Priya AC)"
	)
	assert_almost_eq(
		Stratum1Boss.HIT_PAUSE_HEAVY_DURATION,
		0.100,
		0.0001,
		"HIT_PAUSE_HEAVY_DURATION = 100 ms (Priya AC + VD-07 budget)"
	)
	assert_almost_eq(
		Stratum1Boss.FINAL_FREEZE_DURATION,
		0.300,
		0.0001,
	...
	assert_almost_eq(
		Stratum1Boss.PHASE_TRANSITION_SLOW_MO_DURATION,
		0.60,
		0.0001,
